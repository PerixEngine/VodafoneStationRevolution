...
/////////////////////////////////////////////////////////////////////////////////////////////////
#define DRIVER_VERSION "v2.07.00.01"
...
#define BINTERFACESUBCLASS_NCM 0x0d
///////////////////////////////////////////////////////////////////////////////////////////
...
#define CDC_NCM_NTB_MAX_SIZE_TX 16384  //add by h00122846 for 2.6.21
#endif
...
#ifdef CONFIG_RG_HUAWEI_CDC_ETHER_NCM
	struct mutex cdc_ether_mutex;
	int is_connected;
	wait_queue_head_t chrdev_wait;
	int is_ctrl_msg_chrdev;
	struct mutex ctrl_msg_mutex;
	struct delayed_work ctrl_msg_work;
	u8 *ctrl_msg_buf;
	int ctrl_msg_len;
#endif
};
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,25)
        skb->dev = dev->net;//add by h00122846 for 2.6.21 kernel crash
#endif
...
static int hw_cdc_is_ndis_port_busy(struct hw_cdc_net *hwnet,
	struct usb_interface *intf)
{
	if (HW_JUNGO_BCDDEVICE_VALUE == hwnet->udev->descriptor.bcdDevice ||
		...
		BINTERFACESUBCLASS_NCM == intf->cur_altsetting->desc.bInterfaceSubClass)
	{
		return 0;
	}
	return (hwnet->qmi_sync == 1);
}

static void hw_cdc_set_carrier_status(struct hw_cdc_net *hwnet,
	struct usb_interface *intf, char *buf, int buf_len)
{
	if (buf_len < 17)
		return;
	...
	if (0x04 == buf[6] && 0x22 == buf[9] && 0x00 == buf[10])
	{
		if (0x02 == buf[16])
		{
			if (hwnet)
...
static void hw_delete(struct hw_cdc_net *dev)
{
	struct net_device *net;
	...
	{
		devinfo(dev, "unregister '%s' usb-%s-%s, %s", dev->driver_name,
			xdev->bus->bus_name, xdev->devpath, dev->driver_desc);
	}
...
/*-------------------------------------------------------------------------
 *
 * USB Device Driver CHRDEV support
 *
 *-------------------------------------------------------------------------*/
#ifdef CONFIG_RG_HUAWEI_CDC_ETHER_NCM
#define CHRDEV_LOG_PREFIX "Huawei CDC CHRDEV: "
#define CTRL_MSG_WORK_DELAY (100 * HZ / 1000)
#define CTRL_MSG_LEN 256

#undef CHRDEV_DBG
#ifdef CHRDEV_DBG
#define CHRDBG(format, args...) printk(format, ## args)
#undef KERN_ERR
#define KERN_ERR
#else
#define CHRDBG(format, args...)
#endif

static ssize_t hw_cdc_chrdev_read(struct file *file, char *buffer, size_t count,
	loff_t *ppos)
{
	int ret = 0;
	struct hw_cdc_net *dev = file->private_data;

	mutex_lock(&dev->cdc_ether_mutex);
	mutex_lock(&dev->ctrl_msg_mutex);

	if (!dev->ctrl_msg_len || !count || dev->ctrl_msg_len > count ||
	    !dev->is_connected)
	{
		goto Exit;
	}

	if (copy_to_user(buffer, dev->ctrl_msg_buf, dev->ctrl_msg_len))
	{
		printk(KERN_ERR CHRDEV_LOG_PREFIX "read: copy to user fail\n");
		ret = -EFAULT;
	}
	else
		ret = dev->ctrl_msg_len;

Exit:
	dev->ctrl_msg_len = 0;
	mutex_unlock(&dev->ctrl_msg_mutex);
	mutex_unlock(&dev->cdc_ether_mutex);
	return ret;
}

static ssize_t hw_cdc_chrdev_write(struct file *file, const char *user_buffer,
	size_t count, loff_t *ppos)
{
	...
	hwnet = (struct hw_cdc_net *)dev_get_drvdata(&dev->intf->dev);
	interface = dev->intf->cur_altsetting;

	if (!count)
		goto Exit;

	if (hw_cdc_is_ndis_port_busy(hwnet, dev->intf))
	{
		printk(KERN_ERR CHRDEV_LOG_PREFIX "write: the ndis port is busy\n");
		ret = -EBUSY;
		goto Exit;
	}

	if (!(buf = (char *)kzalloc(count + 1, GFP_KERNEL)))
	{
		printk(KERN_ERR CHRDEV_LOG_PREFIX "write: out of memory\n");
		ret = -ENOMEM;
		goto Exit;
	}
	...
	{
		printk(KERN_ERR CHRDEV_LOG_PREFIX "write: read from user fail\n");
		ret = -EFAULT;
		goto Exit;
	}
	buf[count] = 0;

	ret = usb_control_msg(udev, usb_sndctrlpipe(udev, 0), 0x0, WRITE_REQUEST,
		0x0, interface->desc.bInterfaceNumber, buf, count, 100);
	CHRDBG("%s: write '%s', len %d, ret %d\n", __FUNCTION__, buf, count, ret);
...
static unsigned int hw_cdc_chrdev_poll(struct file *file,
	struct poll_table_struct *wait)
{
	...
	if (!dev || !dev->intf || !dev->is_connected)
	    goto Exit;

	poll_wait(file, &dev->chrdev_wait, wait);

	hwnet = (struct hw_cdc_net *)dev_get_drvdata(&dev->intf->dev);
	if (hwnet && !hw_cdc_is_ndis_port_busy(hwnet, dev->intf))
...
static int hw_cdc_chrdev_open(struct inode *inode, struct file *file)
{
	struct hw_cdc_net *dev;
	...
	if (!(interface = usb_find_interface(&hw_ether_driver, subminor)))
	{
		printk(KERN_ERR CHRDEV_LOG_PREFIX "can't find device for minor %d\n",
			subminor);
		return -ENODEV;
	}
	...
	{
		printk(KERN_ERR CHRDEV_LOG_PREFIX "no context\n");
		return -ENODEV;
	}

	if (dev->is_ctrl_msg_chrdev)
		return -EBUSY;

	file->private_data = dev;
	init_waitqueue_head(&dev->chrdev_wait);
	dev->is_ctrl_msg_chrdev = 1;

	CHRDBG("%s: CHRDEV is opened\n", __FUNCTION__);
	return 0;
}

static int hw_cdc_chrdev_close(struct inode *inode, struct file *file)
{
	int is_connected;
	struct hw_cdc_net *dev = file->private_data;
	...
	dev->is_ctrl_msg_chrdev = 0;
	is_connected = dev->is_connected;
	mutex_unlock(&dev->cdc_ether_mutex);
	
	if (!is_connected)
	{
	    /* Device is disconnected but deletion was postponed - do it now */
	    hw_delete(dev);
	}
...
static const struct file_operations hw_cdc_fops = {
	.owner = THIS_MODULE,
	.read = hw_cdc_chrdev_read,
	.write = hw_cdc_chrdev_write,
	.poll = hw_cdc_chrdev_poll,
	.open = hw_cdc_chrdev_open,
	.release = hw_cdc_chrdev_close,
};

static struct usb_class_driver hw_cdc_class = {
	.name = "huawei_cdc_chrdev%d",
	.fops = &hw_cdc_fops,
	.minor_base = 10,
};

static void hw_cdc_check_ctrl_msg(struct hw_cdc_net *dev)
{
	int ret;
	struct usb_device *udev = interface_to_usbdev(dev->intf);
	struct hw_cdc_net *hwnet =
		(struct hw_cdc_net *)dev_get_drvdata(&dev->intf->dev);
	struct usb_host_interface *interface = dev->intf->cur_altsetting;

	if (hw_cdc_is_ndis_port_busy(hwnet, dev->intf))
	{
		CHRDBG("%s: port is busy\n", __FUNCTION__);
		return;
	}

	/* If previous response yet not read, wait. */
	if (dev->ctrl_msg_len)
	{
		CHRDBG("%s: previous %d bytes yet not read\n", __FUNCTION__,
			dev->ctrl_msg_len);
		return;
	}

	ret = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x01,
		READ_RESPONSE, 0x0, interface->desc.bInterfaceNumber, dev->ctrl_msg_buf,
		CTRL_MSG_LEN, 100);
	if (ret > 0)
	{
		CHRDBG("%s: read '%s', ret %d\n", __FUNCTION__, dev->ctrl_msg_buf,
			dev->ctrl_msg_len);
		hw_cdc_set_carrier_status(hwnet, dev->intf, dev->ctrl_msg_buf,
			dev->ctrl_msg_len);

		/* Awaken char device if exist, ignore the message when not. */
		if (dev->is_ctrl_msg_chrdev)
		{
			dev->ctrl_msg_len = ret;
			wake_up_interruptible(&dev->chrdev_wait);
		}
	}
}

static void hw_cdc_ctrl_msg_work(struct work_struct *work)
{
	struct hw_cdc_net *dev = container_of(work, struct hw_cdc_net,
		ctrl_msg_work.work);

	mutex_lock(&dev->ctrl_msg_mutex);
	hw_cdc_check_ctrl_msg(dev);
	mutex_unlock(&dev->ctrl_msg_mutex);
	schedule_delayed_work(&dev->ctrl_msg_work, CTRL_MSG_WORK_DELAY);
}

static void hw_cdc_ctrl_msg_polling_stop(struct hw_cdc_net *dev)
{
	cancel_rearming_delayed_work(&dev->ctrl_msg_work);

	mutex_lock(&dev->ctrl_msg_mutex);
	kfree(dev->ctrl_msg_buf);
	dev->ctrl_msg_buf = NULL;
	dev->ctrl_msg_len = 0;
	mutex_unlock(&dev->ctrl_msg_mutex);
}

static int hw_cdc_ctrl_msg_polling_start(struct hw_cdc_net *dev)
{
	if (!(dev->ctrl_msg_buf =
		(char *)kzalloc(CTRL_MSG_LEN + 1, GFP_KERNEL)))
	{
		printk(KERN_ERR CHRDEV_LOG_PREFIX "open: out of memory\n");
		return -ENOMEM;
	}

	mutex_init(&dev->ctrl_msg_mutex);
	INIT_DELAYED_WORK(&dev->ctrl_msg_work, hw_cdc_ctrl_msg_work);
	schedule_delayed_work(&dev->ctrl_msg_work, CTRL_MSG_WORK_DELAY);
...
void hw_disconnect (struct usb_interface *intf)
{
	...
	if (dev && dev->is_ncm)
	{
		mutex_lock(&dev->cdc_ether_mutex);
		dev->is_connected = 0;
		is_ctrl_msg_chrdev = dev->is_ctrl_msg_chrdev;
		usb_deregister_dev(intf, &hw_cdc_class);
		mutex_unlock(&dev->cdc_ether_mutex);
		hw_cdc_ctrl_msg_polling_stop(dev);
	}
#endif
	...
	/* If there is a process that holds a file descriptor to the device,
	 * pospone deleting until close() is called.
	 */
	if (!is_ctrl_msg_chrdev)
	    hw_delete(dev);
}
...
#define NCM_MAX_CONTROL_MSG sizeof (*ntb_params)
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,25)
	if (ctx->tx_max_ntb > CDC_NCM_NTB_MAX_SIZE_TX)
    {
            ctx->tx_max_ntb = CDC_NCM_NTB_MAX_SIZE_TX;
    }
#endif
	...
	if (unlikely(ctx->tx_max_ntb < NCM_NTB_MIN_OUT_SIZE)) {
		deverr(ctx->ndev, "dwNtbOutMaxSize (%u) must be at least "
			"%u\n", ctx->tx_max_ntb, NCM_NTB_MIN_OUT_SIZE);
		err = -EINVAL;
		goto exit;
	}
...
/* Add for DTS2011050903736 lxz 20110520 end*/
    ...
    if (HW_JUNGO_BCDDEVICE_VALUE != dev->udev->descriptor.bcdDevice 
     ...
     && BINTERFACESUBCLASS_NCM != udev->cur_altsetting->desc.bInterfaceSubClass) {
		dev->qmi_sync = 0;
	...
	if (dev->is_ncm)
	{
		if ((status = hw_cdc_ctrl_msg_polling_start(dev)))
			goto out3;

		mutex_init(&dev->cdc_ether_mutex);

		if (usb_register_dev(dev->intf, &hw_cdc_class))
		{
			printk(KERN_ERR "Can't open char device");
			status = -ENODEV;
			goto out3;
		}
			...
			void *buf)
{
	...
	if (hw_cdc_is_ndis_port_busy(hwnet, intf))
	{
	...
	switch (req->bRequestType)
	{
		...
		case READ_RESPONSE:
		{
			...
			if (0 < ret){
				hw_cdc_set_carrier_status(hwnet, intf, pbuf, ret);
