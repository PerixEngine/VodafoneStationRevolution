...
extern int brcmnand_auto_markbad;
...
/* XXX
 * The brcmnand_Hamming_WAR seems buggy:
 * It always states the ECC error was in the ECC bytes - even if a bitflip
 * occurs within the inband page data - which leads to overwrite of the accurate
 * HW ECC corrected buffer with a bogus one....
 *
 * Besides, it seems that we do not need brcmnand_Hamming_WAR for the 6362 nand
 * controller; several tests show that the HW ECC correctly fixes 1 bitflip
 * either inband or outband.
 */
#define DONT_USE_HAMMING_WAR 1

#if (DONT_USE_HAMMING_WAR || (CONFIG_MTD_BRCMNAND_VERSION >= CONFIG_MTD_BRCMNAND_VERS_3_1_1))
...
print_oobbuf(oobarea, chip->eccOobSize);}
		...
		ret = 0;
		// No need to ret anything since we mark corrected
...
print_oobbuf(oobarea, chip->eccOobSize);}
		...
		// no need to ret anything, we marked the correction
...
//gdebug=0;
		...
		/* XXX Well, there's no real reason to test for ECC errors during
		 * spare-area-read, they won't be reported...
		 * However, brcmnand_posted_read_oob might call
		 * brcmnand_ctrl_posted_read_cache and do a full page-read
		 * (if >= CONFIG_MTD_BRCMNAND_VERS_3_0) so we'll account these errors.
		 * Either way, outp_oob is filled with spare-area content.
		 */
		if (ret == -EUCLEAN) {
...
static int brcmnand_refresh_blk(struct mtd_info *mtd, loff_t from)
{
	...
	for (i = 0; i < numpages; i++) {
		...
		if (ret < 0 || (!ret && (mtd->ecc_stats.failed - stats.failed))) {
...
//nonBatch = 0;
			...
			if (!aligned || !buffer_aligned || numPages <= 1) {
				...
				if (unlikely(oob)) {
					/* if (ops->mode != MTD_OOB_RAW) */
					oob = brcmnand_transfer_oob(chip, oob, ops, mtd->oobsize);
		...
		while(1) {
			...
			if (unlikely(oob)) {
				...
				if (ops->mode != MTD_OOB_RAW)
				{
				    ...
				    if (toread)
				    {
					oob = brcmnand_transfer_oob(chip, oob, ops, toread);
					oobreadlen -= toread;
				    }
				}
				else {
					buf = brcmnand_transfer_oob(chip, buf, ops, mtd->oobsize);
				}
			}
...
if (gdebug > 3) printk("-->%s: addr=%0llx\n", __FUNCTION__, addr);
	...
	if (!ret && (mtd->ecc_stats.failed - stats.failed))
	    ret = -EIO;
	if (ret) {
 ...
 */
uint8_t *
brcmnand_fill_oob(struct brcmnand_chip *chip, uint8_t *oob, size_t len, struct mtd_oob_ops *ops)
{
	switch(ops->mode) {
	...
	while(1) {
		...
		{ 
			...
			if (unlikely(oob)) {
				...
				chip->oob_poi = BRCMNAND_OOBBUF(chip->ctrl->buffers);
				oob = brcmnand_fill_oob(chip, oob, len, ops);
				oobwritelen -= len;
				/* THT: oob now points to where to read next, 
				 * chip->oob_poi contains the OOB to be written
				 */
...
if (gdebug > 3 ) {
printk("-->%s, to=%08x, ooblen=%d\n", __FUNCTION__, (uint32_t) to, (int)ops->ooblen);}
	...
	if (unlikely(to >= mtd->size ||
	    ops->ooboffs + ops->ooblen > ((device_size(mtd) >> chip->page_shift) -
...
/* The #else case executes in an infinite loop. */
	...
	if (unlikely(oob)) {
		...
		oob = brcmnand_fill_oob(chip, oob, ops->ooblen, ops);
		/* THT: oob now points to where to read next, 
		 * chip->oob_poi contains the OOB to be written
		 */
	}
...
if (gdebug > 3 ) {
printk("-->%s, offset=%0llx, ooblen=%08x\n", __FUNCTION__,  to, (int) ops->ooblen);}
...
static int brcmnand_erase_nolock(struct mtd_info *mtd, struct erase_info *instr, int allowbbt)
{
	...
	while (len) {
		...
		if (!ret)
		{
		    printk (KERN_ERR "%s: brcmnand_ctrl_write_is_complete timedout, addr "
			"0x%08x\n", __FUNCTION__, (unsigned int) addr);
		    instr->state = MTD_ERASE_FAILED;
		    ret = -ETIMEDOUT;
		    goto Exit;
		...
		if (needBBT ) {
			...
			if (!allowbbt && brcmnand_auto_markbad)
			{
