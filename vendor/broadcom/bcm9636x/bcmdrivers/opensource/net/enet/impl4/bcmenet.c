...
void bcm63xx_enet_poll_timer(unsigned long arg);
...
DECLARE_COMPLETION(g_bcmsw_poll_done);
atomic_t g_bcmsw_poll_lock = ATOMIC_INIT(1);
int g_bcmsw_poll_pid = -1;
#define poll_done g_bcmsw_poll_done
#define poll_lock g_bcmsw_poll_lock
#define poll_pid g_bcmsw_poll_pid
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,29)
#else
static const struct softirq_prio_t {
	char type;
	char prio;
	char nice;    
} poll_prio =  /* type, priority, nice */
...
EXPORT_SYMBOL(bcm_ethlock_switch_config);

spinlock_t bcm_ethlock_phy_access;
spinlock_t bcm_extsw_access;
atomic_t phy_access_ref_cnt = ATOMIC_INIT(0);
                                             ...
                                             uint32 context, uint32 free_flag)
{
    ...
    ENET_RX_UNLOCK();
	    ...
	    if (skb->recycle_flags & SKB_DATA_FAP_RECYCLE)
		return;
...
{
#if defined(USE_VPORTS) || defined(CONFIG_HW_ETH_WAN)
    int port;
...
static int create_vport(void)
{
	...
	if (!IsWanPort(phy_id))
	{
	    /* Copy of code called at the cycle bottom for each port */
	    if(pVnetDev0->unit == 0)
	...
	dev->change_mtu             = vnet_dev[0]->change_mtu;
#endif
...
#if defined(IFF_WANDEV) && defined(USE_VPORTS)
		    pNetDev->priv_flags & IFF_WANDEV
#else
		    IsWanPort(phy_id)
#endif
		    );
        ...
        for (i = 1; i <= vport_cnt; i++)
        {
            ...
            if ((priv->linkState & mask) != tmp)
            {
...
#ifdef CONFIG_OPENRG
                        /* (Speed * 990) used to map speeds to 990000, 99000,
                         * and 9900 */
                        bcmPktDma_EthSetPhyRate(port, speed == 1000 ? 0 : 1,
                            speed * 990, IsWanPort(phyId));
#endif
#ifdef USE_VPORTS
                        printk((KERN_CRIT "%s Link UP %d mbps %s duplex\n"),
                                vnet_dev[i]->name, speed, phys.fdx?"full":"half");
#elif defined(CONFIG_OPENRG)
                        printk((KERN_CRIT "%s port %d Link UP %d mbps %s duplex"
                            "\n"), dev->name, port, speed,
                            phys.fdx?"full":"half");
#endif
                    }
                ...
                else
                {
                    ...
                    {
                        int port = vport_to_phyport[i];
                        bcmPktDma_EthSetPhyRate(port, 0, 0, IsWanPort(phyId));
#ifdef USE_VPORTS
                        printk((KERN_CRIT "%s Link DOWN.\n"), vnet_dev[i]->name);
#else
                        printk((KERN_CRIT "%s port %d Link DOWN.\n"), dev->name,
                            port);
#endif
                    }
                }
...
#else
		/* set tag for internal forwarding logic */
        pHdr->brcm_type = 0;
        pHdr->brcm_tag = 0;
#endif
    }
...
#endif
{
        ...
        ) 
    {
        ...
        dmaDesc.status |= DMA_APPEND_BRCM_TAG | (1 << global.pVnetDev0_g->extSwitch->connected_to_internalPort);
#else
        dmaDesc.status = DMA_OWN | DMA_SOP | DMA_EOP | DMA_APPEND_CRC;
#endif
    } else {
...

    {
        int bufSource;
        uint32 key;
        int param1 = 0; /* only used with FAP */
#if defined(CONFIG_BCM_FAP) || defined(CONFIG_BCM_FAP_MODULE)
        BcmEnet_devctrl *pDevCtrl = netdev_priv(dev);
        uint32 destQueue = 0;
	...
	if (skb)
		...
		else
		{
			    ...
			    (skb->recycle_flags & SKB_DATA_RECYCLE))
			{
			    ...
			    bufSource = FAP_ETH_RX;
			    key = (uint32)pdata;
			    param1 = RECYCLE_CONTEXT(skb)->channel;
			    skb->recycle_flags |= SKB_DATA_FAP_RECYCLE;
			    need_skb_free = 1;
			}
		    }
		}
...
tx_continue:
    ...
    /* Prevent update of statistics on bcmsw twice */
    if (pParam->vstats != &global.pVnetDev0_g->stats)
    {
...
static int bcm63xx_enet_poll(struct net_device * dev, int * budget)
{
    ...
    *budget -= work_done;
    ...
    /* Copied fix from bcm63xx_enet_poll_napi above. */
    ENET_RX_LOCK();
...
#if (defined(CONFIG_BCM_FAP) || defined(CONFIG_BCM_FAP_MODULE))
            // This is needed in the FAP version of the driver to keep it from locking up
	    /* XXX There is still a short window between the
	     * bcmPktDma_EthRecvAvailable check above and this line in which a
	     * packet can arrive and won't be handled until a different one
	     * will come after this line. */
            bcmPktDma_EthClrRxIrq(&rxdma->pktDmaRxInfo);
#endif
        }
    }
...
#if defined(CONFIG_HW_ETH_WAN)
	    if (phy_port_id >= MAX_EXT_SWITCH_PORTS && (pDevCtrl->EnetInfo[0].sw.phy_id[phy_port_id] & BCM_WAN_PORT))
		dev = vnet_dev[vport];
	    else
		dev = vnet_dev[0];
#else
...
#ifdef CONFIG_OPENRG
		if (strip_brcm_tag)
		    skb->phy_port_id = (u16)phy_port_id;
#endif
...
static int bcm63xx_init_dev(BcmEnet_devctrl *pDevCtrl)
{
    ...
    for (i = 0; i < cur_rxdma_channels; i++)
    {
        ...
        /* XXX HACK XXX: It appears that when FAP is built in, if a packet (such
         * as an ARP query) is received between this RX enable and the second
         * one (occurring in bcm63xx_enet_open), no further packets will be
         * seen by the driver.
         * Following this, if a broadcast or unicast UDP/TCP packet is sent from
         * a host connected to the switch, the last few received packets are
         * released, and operation proceeds as usual.
         *
         * The workaround in this case is preventing the first RX enable from
         * occurring, apparently closing the problematic window.
         */
#if !(defined(CONFIG_BCM_FAP) || defined(CONFIG_BCM_FAP_MODULE))
...
int __init bcm63xx_enet_probe(void)
{
...
#ifdef NO_CFE
    ethsw_reset(is6829, (int *)&EnetInfo[0].sw.phy_id[0]);
    ...
    dev->change_mtu         = bcm63xx_enet_change_mtu;
...
#else
    dev->features           = NETIF_F_SG | NETIF_F_HW_CSUM | NETIF_F_TSO | NETIF_F_TSO6 | NETIF_F_UFO;
#endif
#endif
    ...
    if (extSwInfo.present) {
        ...
        /* Reset the external switch:
         * bit 4 enables reset and bit 7 activates reset. */
        v8 = 0x90;
        extsw_wreg(PAGE_CONTROL, REG_SW_RESET, &v8, sizeof(v8));
	/* Once RESET is brought high, the PHY will complete its reset sequence
	 * within 5 ms. So wait 10ms to be sure */
        msleep(10);
#endif	
        ...
        case SIOCSWANPORT:
            ...
            if (phyId >= 0) {
                if(IsWanPort(phyId)) {
                    if ((int)data) {
#ifndef CONFIG_OPENRG
			/* We have hardcoded WAN ports in boardparams,
			 * but we still want to initialize them properly. */
                        return 0;
#endif
                    } else {
...
#if defined(CONFIG_BCM_GMAC)
#if (defined(CONFIG_BCM_FAP) || defined(CONFIG_BCM_FAP_MODULE))
int fapDrv_getEnetRxEnabledStatus( int channel );
#endif
