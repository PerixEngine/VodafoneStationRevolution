...
void ethsw_phy_access_disable_interrupt(void);
void ethsw_phy_access_enable_interrupt(void);
...
static PHY_STAT (*switch_port_phy_stat[TOTAL_SWITCH_PORTS])(int phyId);
...
PHY_STAT ethsw_phy_stat(int port)
{
    ...
    if (switch_port_phy_stat[port])
	return switch_port_phy_stat[port](phyId);
...
uint32 ethsw_ephy_auto_power_down_wakeup(void)
{
    ...
    down(&bcm_ethlock_switch_config);
    ethsw_phy_access_disable_interrupt();
    ...
    ethsw_phy_access_enable_interrupt();
    up(&bcm_ethlock_switch_config);
...
#elif defined(ROBOSW250_CLK_EN)
#if 0
	    // Disabled as recommended by Broadcom due to possible risks.
            PERF->blkEnables &= ~ROBOSW250_CLK_EN;
#endif
#endif
        }
...
int ethsw_reset_ports(struct net_device *dev)
{
	...
	ethsw_phy_rreg(phyid, MII_PHYSID1, &phyid1);
        ...
        if ((phyid1 == BCM_PHYID1) &&
	    ((phy_identifier & BCM_PHYID_M) == (BCM54616_PHYID2 & BCM_PHYID_M)))
	{
...
#ifdef NO_CFE
void ethsw_configure_ports(int port_map, unsigned long *pphy_id)
{
    uint8 v8;
    int i;
#if (defined(CONFIG_BCM96816) || defined(CONFIG_BCM96818))
...
EXPORT_SYMBOL(extsw_wreg);

/* This function is used to translate between big endian variables we use on
 * the cpu, and previous pmdio functions that expect different format. */
void extsw_mdio_swap_reg(int reg, u8 *data, int len)
{
    switch (len) {
    case 1:
	break;
    case 2:
	((uint16 *)data)[0] = swab16(((uint16 *)data)[0]);
	break;
    case 4:
...
int extsw_get_accesstype(void)
{
    return pVnetDev0_g->extSwitch->accessType;
}
EXPORT_SYMBOL(extsw_get_accesstype);

void extsw_spi_swap_reg(u8 *data, int len)
{
#define DATA_SWAP(i, j) tmp = data[i];\
    data[i] = data[j];\
    data[j] = tmp;
...
void extsw_swap_reg(int reg, u8 *data, int len)
{
    if (((len != 1) && (len % 2) != 0) || len > 8)
        printk("extsw_swap_reg: wrong length!\n");

    switch (pVnetDev0_g->extSwitch->accessType)
    {
      case MBUS_MDIO:
        extsw_mdio_swap_reg(reg, data, len);
        break;

      case MBUS_SPI:
      case MBUS_HS_SPI:
	extsw_spi_swap_reg(data, len);
        break;
      default:
        printk("Error: Neither SPI nor PMDIO access \n");
        break;
...
EXPORT_SYMBOL(ethsw_phyport_rreg);
...
EXPORT_SYMBOL(ethsw_phyport_wreg);
