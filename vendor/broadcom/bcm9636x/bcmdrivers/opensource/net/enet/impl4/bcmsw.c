...
extern int g_bcmsw_poll_pid;
extern struct completion g_bcmsw_poll_done;
extern atomic_t g_bcmsw_poll_lock;
...
#ifdef NO_CFE
void ethsw_reset(int is6829, int *pphy_id)
{
...
#endif
#ifdef CONFIG_HW_YWZ00B
    /* Reset external phy port and enable auto negotiation */
    v16 = BMCR_RESET | BMCR_ANENABLE | BMCR_ANRESTART;
    ethsw_phy_wreg(pphy_id[4], MII_BMCR, &v16);
#endif
#endif
...
int ethsw_ports_enable(uint32_t port_map, int *pphy_id)
{
#ifdef CONFIG_HW_YWZ00B
    uint16_t v16;
#endif

    /* Take EPHYs out of reset according to port_map */
    GPIO->RoboswEphyCtrl = port_map & 0xf;
    msleep(1);
    ...
    if (g_bcmsw_poll_pid >= 0)
    {
	atomic_set(&g_bcmsw_poll_lock, 0);
	wait_for_completion(&g_bcmsw_poll_done);
	g_bcmsw_poll_pid = -1;
    }

    if (port_map)
    {
	atomic_set(&g_bcmsw_poll_lock, 1);
	g_bcmsw_poll_pid =
	    kernel_thread((int(*)(void *))bcm63xx_enet_poll_timer, 0,
		CLONE_KERNEL);
...
EXPORT_SYMBOL(ethsw_ports_enable);
