...
{
	int i, err;
	dev_t dev = MKDEV(UBI0_MAJOR + ubi->ubi_num, 0);

	sprintf(ubi->ubi_name, UBI_NAME_STR "%d", ubi->ubi_num);

	/*
	 * Major numbers for the UBI character devices are starting from
	 * UBI0_MAJOR. Major numbers of volume character devices are
...
MODULE_LICENSE("GPL");
MODULE_ALIAS_MISCDEV(UBI_CTRL_MINOR);
MODULE_ALIAS_CHARDEV_MAJOR(UBI0_MAJOR);
MODULE_ALIAS_CHARDEV_MAJOR(UBI1_MAJOR);
