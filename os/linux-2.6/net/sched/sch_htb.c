 ...
 *
 * $Id: sch_htb.c,v 1.6.4.3 2015/06/12 06:44:42 mykolak Exp $
 */
			   ...
			   int size)
{
	...
	if (slot > QDISC_RATE_TABLE_SIZE) {
		cl->xstats.giants++;
		slot = QDISC_RATE_TABLE_SIZE;
	}
...
static int htb_requeue(struct sk_buff *skb, struct Qdisc *sch)
{
	...
	if (cl == HTB_DIRECT || !cl) {
		...
		if (q->direct_queue.qlen < q->direct_qlen && cl) {
			...
			sch->qstats.drops_bytes += tskb->len;
			kfree_skb(tskb);
			sch->qstats.drops++;
			return NET_XMIT_CN;
		}
