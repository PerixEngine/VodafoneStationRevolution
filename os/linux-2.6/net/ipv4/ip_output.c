 ...
 *
 * Version:	$Id: ip_output.c,v 1.8.2.5 2015/06/11 14:11:52 mykolak Exp $
 *
 * Authors:	Ross Biro
...
packet_routed:
	...
	iph = (struct iphdr *) skb_push(skb, sizeof(struct iphdr) + (inet_opt ? inet_opt->opt.optlen : 0));
	*((__be16 *)iph) = htons((4 << 12) | (5 << 8) | (inet->tos & 0xff));
	...
	skb->priority = sk->sk_priority;
	skb->mark = sk->sk_mark;
	/* RCU critical section was originally added for dst, which is used in
	 * the NF_HOOK. ip options are not so we can unlock before NF_HOOK. */
	rcu_read_unlock();
		   ...
		   unsigned int flags)
{
	...
	if (skb_queue_empty(&sk->sk_write_queue)) {
		...
		if (ipc_opt) {
			...
			memcpy(inet->cork.opt, &ipc_opt->opt, sizeof(struct ip_options)+ipc_opt->opt.optlen);
