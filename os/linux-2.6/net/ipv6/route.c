 ...
 *
 *	$Id: route.c,v 1.5.16.2 2015/06/11 14:12:04 mykolak Exp $
 *
...
static struct dst_entry *ip6_route_input_lookup(struct flowi *fl, int flags)
{
	if (rt6_need_strict(&fl->fl6_dst))
		flags |= RT6_LOOKUP_F_IFACE;

	return fib6_rule_lookup(fl, flags, ip6_pol_route_input);
}
...
void ip6_route_input(struct sk_buff *skb)
{
	...
	skb->dst = ip6_route_input_lookup(&fl, flags);
}
...
int inet6_rtm_getroute(struct sk_buff *in_skb, struct nlmsghdr* nlh, void *arg)
{
	...
	if (iif) {
		...
		rt = (struct rt6_info *)ip6_route_input_lookup(&fl, flags);
	} else {
		fl.oif = oif;
