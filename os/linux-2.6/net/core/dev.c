...
struct net_hooks_t net_hooks = {
	.rx_pre_header_modify = net_hook_stub,
	.rx_pre_qos = net_hook_stub,
	.rx_pre_handlers = net_hook_stub,
	.tx_pre_qos = net_hook_stub,
	.tx_pre_driver = net_hook_stub,
};

EXPORT_SYMBOL(net_hooks);

void net_hooks_set(struct net_hooks_t *hooks)
{
#define HOOK_ASSIGN(hkfn) (net_hooks.hkfn = hooks->hkfn ?: net_hook_stub)
	HOOK_ASSIGN(rx_pre_header_modify);
	HOOK_ASSIGN(rx_pre_qos);
	HOOK_ASSIGN(rx_pre_handlers);
	HOOK_ASSIGN(tx_pre_qos);
	HOOK_ASSIGN(tx_pre_driver);
#undef HOOK_ASSIGN
}
EXPORT_SYMBOL(net_hooks_set);
...
extern int ti_dev_new_index(void);
...
int dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	if (likely(!skb->next)) {
		...
		if (!net_hooks.tx_pre_driver(&skb))
			return NETDEV_TX_OK;
...
int netif_rx_packet_count(void)
{
	int i;
	int count = 0;

	for (i = 0; i < NR_CPUS; i++)
		count += per_cpu(netdev_rx_stat, i).total;

	return count;
}
EXPORT_SYMBOL(netif_rx_packet_count);
...
{
	struct net_device *orig_dev;
	unsigned int net_rx_lower_flags = 0;
	...
	if (skb->tc_verd & TC_NCLS)
		net_rx_lower_flags = NET_RX_LOWER_NCLS;
#endif

	if (likely(!net_rx_lower_flags) &&
	    !net_hooks.rx_pre_header_modify(&skb)) {
		return NET_RX_SUCCESS;
	}
	...
	return net_rx_lower(skb, orig_dev, net_rx_lower_flags);
}

int net_rx_lower(struct sk_buff *skb, struct net_device *orig_dev,
		 unsigned int flags)
{
	...
	{
#ifdef CONFIG_NET_CLS_ACT
		if (flags & NET_RX_LOWER_NCLS) {
			skb->tc_verd = CLR_TC_NCLS(skb->tc_verd);
			goto ncls;
		}
#endif
		if (flags & NET_RX_LOWER_REINJECT_BY_HOOKS)
			goto pre_qos;
	}
...
ncls:
	...
	if (!net_hooks.rx_pre_handlers(&skb)) {
		ret = NET_RX_SUCCESS;
		goto out;
	}
