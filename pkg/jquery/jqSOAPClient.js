...
var SOAPClient = {
	...
	SendRequest: function(soapReq, callback, timeout) {		
		if(SOAPClient.Proxy != null) {
			...
			function getResponse(xData, textStatus) {
				if(callback != null) {
					if (textStatus != "timeout") {
						SOAPClient.Status = xData.status;
/* JUNGO: Removed the original xml to JSon - We don't want it
 * Also added use of timeout and textStatus */
/*
...
*/					
					...
					if (!jqsoap_unloaded)
						callback(xData.responseXML, textStatus);
				}
			}
			$.ajax({
				 type: "POST",
				 ...
				 beforeSend: function(req) {
					...
					// No need to sent Content-Length, modern browsers assumes it as
					// attempt of attack
