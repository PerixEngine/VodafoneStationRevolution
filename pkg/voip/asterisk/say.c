		...
		*/
		if (num < 0) {
			...
			if (ones) {
				...
				snprintf(fna, sizeof(fna), "digits/and");
				num -= ones;				
			} else {
				snprintf(fn, sizeof(fn), "digits/%d", num);
				num = 0;
...
static int ast_say_enumeration_full_de(struct ast_channel *chan, int num, const char *ints, const char *language, const char *options, int audiofd, int ctrlfd)
{
	...
	while(!res && num) {
		if (num < 0) {
			...
			snprintf(fn, sizeof(fn), "digits/%d%s", num, gender);
			num = 0;
		} else if (num < 100) {
			int ones = num % 10;
			if (ones) {
			...
			snprintf(fn, sizeof(fn), "digits/hundred%s", gender);
			num = 0;
		} else if (num < 1000) {
			int hundreds = num / 100;
			num = num % 100;
			...
			if (num) {					
				...
				snprintf(fna, sizeof(fna), "digits/hundred%s", gender);
			}
			t = 1;
		} else 	if (num < 1000000) {
			int thousands = num / 1000;
		...
		switch (format[offset]) {
			...
			case 'Y':
				/* Year */
				{
					int year = tm.tm_year + 1900;
					if (year > 1999) {	/* year 2000 and later */
						snprintf(nextmsg, sizeof(nextmsg), "digits/2000");
						res = wait_file(chan, ints, nextmsg, lang);
						if (res)
							break;
						year -= 2000;
			...
			case 'H':
			case 'k':
				ast_copy_string(nextmsg, "digits/at", sizeof(nextmsg));
				res = wait_file(chan,ints,nextmsg,lang);
				if (res)
					break;
				/* 24-Hour */
