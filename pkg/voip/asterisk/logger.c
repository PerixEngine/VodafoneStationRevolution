...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.14.10.3 $")
...
static int syslog_level_map[] = {
	...
	LOG_DEBUG,
	LOG_DEBUG,
	LOG_INFO    /* LOG_MFT_INFO */
};

#define SYSLOG_NLEVELS 8
...
char *logger_hide_numbers(char *str, int min_len_to_hide)
{
	int i, digit_seq_len = 0;
	char *c, last_non_digit = 0;

	if (!logfiles.hide_numbers)
		return str;
	...
	for (c = str; *c; c++)
	{
		...
		if ((last_non_digit == ':' && strchr(" :;>\n\r", *(c+1))) ||
			(last_non_digit == '=' && strchr(" \n\r", *(c+1))) ||
			last_non_digit == '_' || *(c+1) == '_' ||
			last_non_digit == '.' || *(c+1) == '.' ||
			isalpha(last_non_digit) || isalpha(*(c+1)))
		{
			/* This digit sequence is probably an IP address, a port number,
			 * or a random string */
			continue;
		}

		digit_seq_len++;
		if (!isdigit(*(c+1)) && digit_seq_len >= min_len_to_hide)
		{
			/* We're at the end of a digit sequence which must be hidden */
			for (i = 0; i < digit_seq_len; i++)
				*(c-i) = 'x';
		}
	}
	return str;
...
char logger_hide_number(char c)
{
	if (!logfiles.hide_numbers || !isdigit(c))
		return c;
...
static int make_components(char *s, int lineno)
{
	...
	while(w) {
		...
		if (!strcasecmp(w, "error")) 
			...
			res |= (1 << __LOG_DTMF);
		else if (!strcasecmp(w, "mft_info"))
			res |= (1 << __LOG_MFT_INFO);
		else {
...
static struct logchannel *make_logchannel(char *channel, char *components, int lineno)
{
	...
	if (!strcasecmp(channel, "console")) {
		...
		*/
		...
		rg_openlog("asterisk", LOG_PID | LOG_NDELAY, chan->facility);
	} else {
		if (channel[0] == '/') {
...
static void init_logger_chain(void)
{
	...
	if ((s = ast_variable_retrieve(cfg, "general", "hide_numbers"))) {
		logfiles.hide_numbers = ast_true(s);
	}
...
#define FORMATL	"%-35.35s %-8.8s %-9.9s "
	...
	while (chan) {
		...
		if (chan->logmask & (1 << __LOG_EVENT)) 
			ast_cli(fd, "Event ");
		if (chan->logmask & (1 << __LOG_MFT_INFO))
			ast_cli(fd, "Manufacturing_Info ");
		ast_cli(fd, "\n");
		chan = chan->next;
	}
	ast_cli(fd, "\n");
...
static void ast_log_syslog(int level, const char *file, int line, const char *function, const char *msg) 
{
	...
	s = buf + strlen(buf);
	strncpy(s, msg, sizeof(buf) - strlen(buf));
	strip_coloring(s);
	syslog(syslog_level_map[level], "%s", buf);
}

static char *ast_vsnprintf(char *msg, int max_len, const char *fmt, va_list ap)
{
	vsnprintf(msg, max_len, fmt, ap);
	logger_hide_numbers(msg, 3);
...
void ast_log(int level, const char *file, int line, const char *function, const char *fmt, ...)
{
	...
	*/
	...
	if (logfiles.event_log && level == __LOG_EVENT) {
		...
		ast_vsnprintf(buf, sizeof(buf), fmt, ap);
		fputs(buf, eventlog);
		fflush(eventlog);
	...
	if (logchannels) {
		...
		while(chan && !chan->disabled) {
			...
			if (chan->type == LOGTYPE_SYSLOG && (chan->logmask & (1 << level))) {
				va_start(ap, fmt);
				ast_vsnprintf(buf, sizeof(buf), fmt, ap);
				ast_log_syslog(level, file, line, function, buf);
				va_end(ap);
			/* Console channels */
...
void ast_verbose(const char *fmt, ...)
{
	...
	*/
	ast_vsnprintf(stuff + len, sizeof(stuff) - len, fmt, ap);
	va_end(ap);
