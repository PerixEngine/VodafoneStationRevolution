...
/*!\brief Retrieve set of SRV lookups, in order, skips stale entries
 * \param[in] context A pointer in which to hold the result
 * \param[in] service The service name to look up
 * \param[in] min_ttl Assume at least min_ttl on each result
 * \param[out] host Result host
 * \param[out] port Associated TCP portnum
 * \param[out] ttl TTL of the result
 * \retval -1 Query failed
 * \retval 0 Result exists in host and port
 * \retval 1 No more results
 */
extern int ast_srv_lookup(struct srv_context **context, const char *service, int min_ttl, const char **host, unsigned short *port, int *ttl);
