...
#define AST_MAX_SHARED_CALLID   80

#define AST_MAX_FILE_NAME	256
...
struct ast_channel_tech {
	...
	const char * const call_description;
	...
	/*! Prepare channel for bridging */
	void (* const pre_bridge)(struct ast_channel *chan);

	/*! Free resources allocated for bridge */
	void (* const post_bridge)(struct ast_channel *chan);

	/*! Return RTP session used by the channel */
	struct ast_rtp *(* const get_rtp)(struct ast_channel *chan);

	/*! Return UDPTL session used by the channel */
	struct ast_udptl *(* const get_udptl)(struct ast_channel *chan);

	/*! Attended transfer */
	int (* const attendedtransfer)(struct ast_channel *chan, struct ast_channel *target);

	/*! \brief Get the unique identifier for the PVT, i.e. SIP call-ID for SIP */
	char *(* get_pvt_uniqueid)(struct ast_channel *chan);

        /*! Return our IP we'll use in a conversation */
	struct in_addr (*get_ourip)(struct ast_channel *chan);
};
...
struct ast_channel {
	...
	/*! Raw write format */
	int rawwriteformat;
	/*! Indicates whether the channel is an extension of template enabled trunk */
	int is_template_trunk_extension;
	char template_caller_name[AST_CHANNEL_NAME];
	
	/*! Currently is used by chan_sip only */
	void *answer_data;

	/*! Callid shared by channels in the same call - used for DSP data channel allocation */
...
#define AST_FLAG_IN_AUTOLOOP	(1 << 9)	/*!< the channel is in an auto-incrementing dialplan processor,
						...
						*/
#define AST_FLAG_CALL_ONHOLD	(1 << 10)	/*!< channel is on hold */

#define AST_FLAG_T38            (1 << 11)       /*!< the channel in t38 mode */
/*! A DTMF_BEGIN frame has been read from this channel, but not yet an END */
#define	AST_FLAG_IN_DTMF       (1 << 12)
/* @} */
#define	AST_FLAG_OFFHOOK       (1 << 13)
#define	AST_FLAG_CALLBACK      (1 << 14)
#define	AST_FLAG_CALLWAIT      (1 << 15)
#define	AST_FLAG_ANSWERED_ELSEWHERE (1 << 16)
#define AST_FLAG_FORCED_READ   (1 << 17)
#define AST_FLAG_DISALLOW_AUDIO_JUMPERS   (1 << 18)
...
/*! Get the channel that owns the given rtp session */
struct ast_channel *ast_get_channel_owner_of_rtp(struct ast_rtp *rtp);

/*! Get the channel that owns the given rtp session or its bridged channel,
    if the first does not have a CDR struct */
...
/* This function works like ast_readstring only escape code was added
* \param full_s Original pointer to array where the DTMF data will be stored
* \param escape_code, when escape_code==s then timeout will become escape_timeout
* \param escape_timeout timeout when escape_code==s.  
* \param break_keys When break_key is typed function will return this key immediatelly.  
* \return
*    <0 if an error occurred
*     0 if user dialed a character from "enders" or "break_keys"
*     1 if a timeout occurred
*     3 if a number from "escape_code" was dialed
*
* Designed to allow changing timeout when escape code was typed and handle break
*/
int ast_readstring_allow_escape(struct ast_channel *c, char *s, int len, int timeout, int ftimeout, char *enders, char *full_s, char *escape_code, int escape_time_out, char *break_keys);
 ...
 * \param is_onhold indicates if channel is on_hold
...
static inline int ast_select(int nfds, fd_set *rfds, fd_set *wfds, fd_set *efds, struct timeval *tvp)
{
	...
	if (tvp) {
		...
		tv = *tvp;
		ast_time_from_boot(&tvstart);
		res = select(nfds, rfds, wfds, efds, tvp);
		ast_time_from_boot(&tvend);
