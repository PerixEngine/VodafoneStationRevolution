...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.42.4.3.2.26.2.1 $")
...
char *ast_state2str(int state, int is_onhold)
{
...
int ast_queue_control_data(struct ast_channel *chan, int control, const void *data, size_t datalen)
{
	struct ast_frame f = { AST_FRAME_CONTROL, };
...
struct ast_channel *ast_get_channel_owner_of_rtp(struct ast_rtp *rtp)
{
    struct ast_channel *chan, *nextchan;

    nextchan = ast_channel_walk_locked(NULL);
    while (nextchan)
    {
        if (nextchan->tech && nextchan->tech->get_rtp &&
	        (rtp == nextchan->tech->get_rtp(nextchan)))
	{
	    break;
	}

        chan = nextchan;
	nextchan = ast_channel_walk_locked(chan);
        ast_mutex_unlock(&chan->lock);
    }
...
struct ast_channel *ast_get_cdr_channel_owner_of_rtp(struct ast_rtp *rtp)
{
    struct ast_channel *chan;

    chan = ast_get_channel_owner_of_rtp(rtp);
    ...
    if (!chan->cdr)
    {
        struct ast_channel *chan_bridge;

	chan_bridge = ast_bridged_channel(chan);
        ast_mutex_unlock(&chan->lock);
	chan = chan_bridge;
	if (chan_bridge)
	    ast_mutex_lock(&chan_bridge->lock);
    }
...
{
	int res = 0, hangupcause;
	char name[AST_CHANNEL_NAME], uniqueid[32];
	...
	if (chan->cdr) {		/* End the CDR if it hasn't already */ 
		...
		ast_cdr_hangupcause(chan->cdr, chan->hangupcause);
...
int ast_waitfor_n_fd(int *fds, int n, int *ms, int *exception)
{
	...
	if (*ms > 0) {
		*ms -= ast_tvdiff_ms(ast_tvfromboot(), start);
		if (*ms < 0)
			*ms = 0;
	}
	return winner;
		...
		if (ms < 0) /* Error */
			...
			if (f) {
				if (f->frametype == AST_FRAME_DTMF) 
				    ...
				    f->subclass == AST_CONTROL_CALLBACK)
				{
				    result = AST_RESULT_CALLBACK;
				}
				ast_frfree(f);
			} else
				result = -1;
...
struct ast_frame *ast_read(struct ast_channel *chan)
{
	...
	if (!ast_test_flag(chan, AST_FLAG_DEFER_DTMF | AST_FLAG_IN_DTMF) && !ast_strlen_zero(chan->dtmfq)) {
	...
	if (chan->readq) {
		...
		if (ast_test_flag(chan, AST_FLAG_EXCEPTION)) {
			...
			ast_clear_flag(chan, AST_FLAG_EXCEPTION);
		        ...
		        if (ast_test_flag(chan, AST_FLAG_FORCED_READ))
				f = &null_frame;
			else
			{
				if (chan->tech->read)
	...
	if (f && chan->generatordata) {
		 ...
		 * 1) We think it makes sense that if you can't hear the
		 *    other side, he can't hear you as well.
		 * 2) Sending voice frames during jrtp unbinding will
		 *    screw things up. */
		ast_frfree(f);
		f = &null_frame;
	...
	if (!chan->tech->indicate || res) {
		...
		if (condition >= 0) {
			...
			if (ts && ts->data[0]) {
				...
				/* Do nothing.... */
			} else if (condition == AST_CONTROL_FLASH) {
			    	/* Do nothing.... */
			} else {
				/* not handled */
...
int ast_senddigit_begin(struct ast_channel *chan, char digit)
{
	...
	/* JUNGO: Since our embedded systems are too weak to do codec
	 * conversion, we cannot do tone generation. We rely on the DSP to pass
	 * the DTMF in the voice path. */
#if 0
...
int ast_write(struct ast_channel *chan, struct ast_frame *fr)
{
	...
	case AST_FRAME_TEXT:
		if (chan->tech->send_text)
			...
			res = 0;
		break;
	case AST_FRAME_BLINDTRANSFER:
		if (chan->tech->transfer)
			res = chan->tech->transfer(chan, (char *) fr->data);
		else
			res = 0;
		break;
	case AST_FRAME_ATTENDEDTRANSFER:
		if (chan->tech->attendedtransfer)
			res = chan->tech->attendedtransfer(chan, (struct ast_channel *) *((struct ast_channel **)(fr->data)));
		else
			res = 0;
		break;
	case AST_FRAME_HTML:
	...
	} else if (fr->frametype == AST_FRAME_DTMF_BEGIN || fr->frametype == AST_FRAME_DTMF_END) {
		      ...
		      const int direction)
{
	...
	native = ast_codec_pref_bits(&chan->nativeformats);
	/* Find a translation path from the native format to one of the desired formats */
	if (!direction)
		/* reading */
		res = ast_translator_best_choice(&fmt, &native, NULL);
	else
		/* writing */
		res = ast_translator_best_choice(&native, &fmt, NULL);
	...
	for (chan = backends; chan; chan = chan->next) {
		...
		if (c->_state == AST_STATE_DOWN) {
			manager_event(EVENT_FLAG_CALL, "Newchannel",
				      ...
				      "CallerIDName: %s\r\n"
				      "Uniqueid: %s\r\n",
				      c->name, ast_state2str(c->_state, ast_test_flag(c, AST_FLAG_CALL_ONHOLD)),
...
{
    return ast_readstring_allow_escape(c, s, len, timeout, ftimeout, enders, NULL, NULL, 0, NULL);
}

static int is_escape_code(char *escape_codes, char *str)
{
	char *buf, *curr, *next;
	int rc = 0;
	...
	buf = strdup(escape_codes);
	for (curr = buf; curr; curr = next) {
		if ((next = strchr(curr, '&'))) {
			*next = '\0';
			next++;
		}
...
int ast_readstring_allow_escape(struct ast_channel *c, char *s, int len, int timeout, int ftimeout, char *enders, char *full_s, char *escape_code, int escape_timeout, char *break_keys)
{
	int pos=0;
	int to = ftimeout;
	int d;
	int escape_code_dialed = 0;
	...
	do {
		...
		if (d == 0) {
			s[pos]='\0';
			return escape_code_dialed ? 3 : 1;
		}
		if (!strchr(enders, d))
			s[pos++] = d;
		if (break_keys && strchr(break_keys, d))
		{ 
		        s[0] = d;
		        s[1] = '\0';
		        return 0;
		...
		to = timeout;
		if ((escape_code_dialed = is_escape_code(escape_code, full_s)))
			to = escape_timeout;
	} while(1);
	/* Never reached */
	return 0;
...
/* Assumes indices are between 0 to 31, cost is between 0 to 99999 */
static unsigned int calc_path_total_cost(int pref_index_1, int pref_index_2, int cost)
{
	/* Large penalty for transcoding, small pentalties for codec order */
    	return 1024*cost + 32*pref_index_1 + pref_index_2;
}
...
int ast_channel_make_compatible(struct ast_channel *chan, struct ast_channel *peer)
{
	...
	int res = -1;
	int i, j;
	int transcode_cost, bestsrc, bestdst;
	unsigned int cost, bestcost;
	struct ast_channel *src_channel, *dst_channel;

	if (peer->preferred_codec) {
	    src_channel = chan;
	    dst_channel = peer;
	} else {
	    dst_channel = chan;
	...
	bestcost = UINT_MAX;
	for (i = 0; i < 32; ++i) {
		dst = ast_codec_pref_index_audio(&dst_channel->nativeformats, i);
		if (!dst) break;
		for (j = 0; j < 32; ++j) {
			src = ast_codec_pref_index_audio(&src_channel->nativeformats, j);
			if (!src) break;
			res = ast_translator_best_choice(&dst, &src, &transcode_cost);
			if (!res)
			{
				cost = calc_path_total_cost(i, j, transcode_cost);
				if (cost < bestcost)
				{
					bestcost = cost;
					bestsrc = src;
	...
	/* Get packetization time from the channel that determined the read format */
	if (chan->readformat == chan->nativeformats.audio_bits && chan->ptime)
	    peer->ptime = chan->ptime;
	else
	    chan->ptime = peer->ptime;
...
int ast_channel_masquerade(struct ast_channel *original, struct ast_channel *clone)
{
	...
	if (!original)
	{
	    	ast_log(LOG_WARNING, "masquerade channel is NULL\n");
	    	return -1;
	}
	if (original == clone) {
...
int ast_do_masquerade(struct ast_channel *original)
{
	...
	if (ast_test_flag(clone, AST_FLAG_ZOMBIE)) {
		...
		strcpy(name, clone->name);
		ast_copy_string(uniqueid, clone->uniqueid, sizeof(uniqueid));
		hangupcause = clone->hangupcause;
		ast_mutex_unlock(&clone->lock);
	...
	if (ast_bridged_channel(clone))
		ast_queue_hangup(ast_bridged_channel(clone));
...
int ast_setstate(struct ast_channel *chan, int state)
{
	...
	manager_event(EVENT_FLAG_CALL,
		      ...
		      "CallerIDName: %s\r\n"
		      "Uniqueid: %s\r\n",
		      chan->name, ast_state2str(chan->_state, ast_test_flag(chan, AST_FLAG_CALL_ONHOLD)), 
						 ...
						 struct ast_channel **rc, struct timeval bridge_end)
{
	...
	for (;;) {
		...
		gen_active = c0->generatordata || c1->generatordata;
		if (gen_active)
		    to = 1;
		else
		{
		    if (bridge_end.tv_sec) {
			to = ast_tvdiff_ms(bridge_end, ast_tvfromboot());
			if (to <= 0) {
			    res = AST_BRIDGE_RETRY;
			    break;
			}
		...
		if ((!who && gen_active) || (who != *cs && (*cs)->generatordata))
		{		    
		    who = (*cs)->generatordata ? *cs : (*cs == c0 ? c1 : c0);
		    ast_set_flag(who, AST_FLAG_FORCED_READ);
		}
		else if (!who) {
		...
		f = ast_read(who);
		ast_clear_flag(who, AST_FLAG_FORCED_READ);
		if (!f) {
			*fo = NULL;
			*rc = who;
		...
		if ((f->frametype == AST_FRAME_CONTROL) && !(config->flags & AST_BRIDGE_IGNORE_SIGS)) {
			if ((f->subclass == AST_CONTROL_HOLD) || (f->subclass == AST_CONTROL_UNHOLD) ||
			    ...
			    (f->subclass == AST_CONTROL_FAX) ||
			    (f->subclass == AST_CONTROL_MODEM) ||
			    (f->subclass == AST_CONTROL_T38) ||
			    (f->subclass == AST_CONTROL_FLASH) ||
				(f->subclass == AST_CONTROL_ANSWER) ||
				(f->subclass == AST_CONTROL_TRANSPARENT_DATA)) {
				ast_indicate_data(who == c0 ? c1 : c0, f->subclass, f->data, f->datalen);
			} else {
				*fo = f;
				*rc = who;
				/* subclass -1 is used as a type of null frame.
				 * make sure we don't lose codec changes because of this */
				res =  f->subclass == -1 ? AST_BRIDGE_RETRY : AST_BRIDGE_COMPLETE;
		...
		if ((f->frametype == AST_FRAME_VOICE) ||
			...
			if (f->frametype == AST_FRAME_DTMF_END || 
				...
				if (((who == c0) && watch_c0_dtmf) ||
					...
					*fo = f;
					res = AST_BRIDGE_COMPLETE;
					ast_log(LOG_DEBUG, "Got DTMF %s on channel (%s)\n", f->frametype == AST_FRAME_DTMF_END ? "end" : "begin", who->name);
					break;
				} else {
					goto tackygoto;
				}
...
void ast_channel_pre_bridge(struct ast_channel *c0, struct ast_channel *c1)
{
	if (!ast_test_flag(c0, AST_FLAG_ZOMBIE) && c0->tech->pre_bridge)
		c0->tech->pre_bridge(c0);

	if (!ast_test_flag(c1, AST_FLAG_ZOMBIE) && c1->tech->pre_bridge)
		c1->tech->pre_bridge(c1);
}

void ast_channel_post_bridge(struct ast_channel *c0, struct ast_channel *c1)
{
	if (!ast_test_flag(c0, AST_FLAG_ZOMBIE) && c0->tech->post_bridge)
		c0->tech->post_bridge(c0);

	if (!ast_test_flag(c1, AST_FLAG_ZOMBIE) && c1->tech->post_bridge)
		c1->tech->post_bridge(c1);
}
					  ...
					  struct ast_bridge_config *config, struct ast_frame **fo, struct ast_channel **rc) 
{
	...
	*fo = NULL;
	...
	if (ast_tvzero(config->start_time))
		config->start_time = ast_tvfromboot();
...
retrylock:
...
#if defined(T38_SUPPORT)
	if (ast_test_flag(c0, AST_FLAG_T38) || ast_test_flag(c1, AST_FLAG_T38)) {
		ast_log(LOG_NOTICE, "T38: Channels %s and %s\n", c0->name, c1->name);
	} else
#endif
	...
	for (/* ever */;;) {
		...
		if (c0->tech->get_pvt_uniqueid)
			pbx_builtin_setvar_helper(c1, "SIPCALLID", c0->tech->get_pvt_uniqueid(c0));
		if (c1->tech->get_pvt_uniqueid)
			pbx_builtin_setvar_helper(c0, "SIPCALLID", c1->tech->get_pvt_uniqueid(c1));
		if (c0->tech->bridge &&
		    (config->timelimit == 0) &&
		...
		ast_channel_pre_bridge(c0, c1);
		res = ast_generic_bridge(c0, c1, config, fo, rc, nexteventts);
		ast_channel_post_bridge(c0, c1);
...
void ast_run_openrg_cmd(char *cmd)
{
