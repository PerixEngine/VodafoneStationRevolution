...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.5.4.1 $")
...
const unsigned char FILE_VER = 0x01;
...
static struct ast_filestream *g729_open(FILE *f)
{
	...
	/* We need to get past the header, which currently is the file
	 * version. */
	fseek(f, sizeof(g729_header_t), SEEK_SET);
...
{
	struct ast_filestream *tmp;
	g729_header_t hdr = { FILE_VER };

	/* Writing the header, which is the file version. */
...
static struct ast_frame *g729_read(struct ast_filestream *s, int *whennext)
{
	...
	if ((res = fread(&frame_size, 1, sizeof(frame_size), s->f)) !=
	    sizeof(frame_size)) {
		return NULL;
	}
	s->fr.datalen = frame_size;

	if ((res = fread(s->g729, 1, frame_size, s->f)) != frame_size) {
		if (res && (res != 10))
...
static int g729_write(struct ast_filestream *fs, struct ast_frame *f)
{
	...
	/* G729 Annex B support. */
	frame_size = f->datalen;
	if ((res = fwrite(&frame_size, 1, sizeof(frame_size), fs->f)) != sizeof(frame_size))
	{
		ast_log(LOG_WARNING, "Could not write frame header.\n");
		return -1;
	}
