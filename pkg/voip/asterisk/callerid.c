...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.5.2.1 $")
...
char *ast_callerid_adjust(char *caller_id)
{
#ifdef CALLER_ID_PREFIX
    static char adj_caller_id[40];
    int off;

    if (!caller_id)
	return NULL;

    /* if number starts with '00', '+' or '+00', replace it with prefix */
    if (caller_id[0] == '+')
    {
	if (caller_id[1] == '0' && caller_id[2] == '0')
	    off = 3;
        else
	    off = 1;
    }
    else if (caller_id[0] == '0' && caller_id[1] == '0')
	off = 2;
    else
	return caller_id;

    snprintf(adj_caller_id, sizeof(adj_caller_id), CALLER_ID_PREFIX "%s",
	&caller_id[off]);

    return adj_caller_id;
#else
    return caller_id;
#endif
}
