...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.7.6.1.2.2 $")
...
int ast_sched_wait(struct sched_context *con)
{
	...
	if (!con->schedq) {
		...
		ms = ast_tvdiff_ms(con->schedq->when, ast_tvfromboot());
		if (ms < 0)
			ms = 0;
	}
	ast_mutex_unlock(&con->lock);
...
int ast_sched_del_and_get_data(struct sched_context *con, int id, void **data)
{
	/*
	...
	DEBUG(ast_log(LOG_DEBUG, "ast_sched_del_and_get_data()\n"));
	ast_mutex_lock(&con->lock);
	s = con->schedq;
	while(s) {
		if (s->id == id) {
	...
	if (!s && con->currently_executing && (id == con->currently_executing->id)) {
...
int ast_sched_del(struct sched_context *con, int id)
{
	...
	return ast_sched_del_and_get_data(con, id, &dummy);
}
...
int ast_sched_runq(struct sched_context *con)
{
	...
	ast_mutex_lock(&con->lock);
	tv = ast_tvadd(ast_tvfromboot(), ast_tv(0, 1000));
	for(;;) {
		if (!con->schedq)
			break;
