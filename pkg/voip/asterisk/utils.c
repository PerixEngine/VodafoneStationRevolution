...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.8.2.5 $")
...
static int s_so_mark;

void ast_set_so_mark(int mark)
{
	s_so_mark = mark;
}

int ast_get_so_mark(void)
{
	return s_so_mark;
}
...
struct hostent *ast_gethostbyname(const char *host, struct ast_hostent *hp)
{
	...
	if (!result || !hp->hp.h_addr_list || !hp->hp.h_addr_list[0])
		return NULL;
#else
	res = rg_gethostbyname_r(host, &hp->hp, hp->buf, sizeof(hp->buf), &result, &herrno, s_so_mark);
...
struct timeval ast_tvfromboot (void)
{
    ...
    clock = (times(&tmp_tms) - first);

    t.tv_sec = clock / ticks_per_sec;
    t.tv_usec = (clock - (t.tv_sec * ticks_per_sec)) * (1000000 /
        ticks_per_sec);
...
void ast_hexencode(char *src, char *dst, int len)
{
	for (; len; src++, len--)
	{
		*dst++ = VAL2HEX(*src>>4 & 0xf);
		*dst++ = VAL2HEX(*src & 0xf);
	}
	*dst = 0;
}

int ast_hexdecode(char *src, char *dst, int max)
{
	char tmp[3];
	int len = 0;
...
/* Return 1 iff saved_md5 differs from the md5 digest of filename */
int ast_config_file_md5_update(const char *filename, unsigned char *saved_md5)
{
	char fullname[256];
	FILE *f;
	int retval = 0;
	unsigned char new_md5[MD5_DIGEST_LEN] = {};

	snprintf(fullname, sizeof(fullname), "%s/%s", (char *)ast_config_AST_CONFIG_DIR, filename);
	f = fopen(fullname, "r");
	if (f)
	{
		struct MD5Context md5;
		...
		MD5Final(new_md5, &md5);

		fclose(f);
	}

	if (memcmp(saved_md5, new_md5, MD5_DIGEST_LEN))
	    retval = 1;

	memcpy(saved_md5, new_md5, MD5_DIGEST_LEN);
