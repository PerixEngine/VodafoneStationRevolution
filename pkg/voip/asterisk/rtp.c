...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.35.2.2.2.20.4.1 $")
...
void ast_rtp_set_peer(struct ast_rtp *rtp, struct sockaddr_in *them, int mode)
{
	rtp_ioctl_connect_t params;

	params.port = ntohs(them->sin_port);
	params.addr = them->sin_addr.s_addr;
...
static int rtp_stats_cb(void *data)
{
        ...
        if (chan && (cdr = chan->cdr))
	{
	    ...
	    else
	    {
	        ...
	        cdr->them = addrs.dest_addr;
		cdr->us = addrs.src_addr;
		...
		if (!sip_chan->tech || !sip_chan->tech->get_ourip)
		    ...
		    cdr->us.sin_addr = sip_chan->tech->get_ourip(sip_chan);
	    }
	}
