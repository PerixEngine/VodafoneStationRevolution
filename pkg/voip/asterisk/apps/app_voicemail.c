...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.9.4.28 $")
...
#include <voip/dbm/voicemail_dbm.h>
#include <voip/dbm/voicemail_common_utils.h>
#endif
...
*/
#if defined(USE_SQLITE_DB)
voicemail_dbm_t *dbm = NULL; 
AST_MUTEX_DEFINE_STATIC(vm_dbm_lock);
#endif
...
#endif
#if defined(USE_SQLITE_DB)
#define OP_GET_NEXT_MSG "GET_NEXT_MSG"
#define OP_GET_FIRST_MSG "GET_FIRST_MSG"
#define OP_GET_PREV_MSG "GET_PREV_MSG"
#define OP_GET_MSG_COUNT "GET_MSG_COUNT"
#define OP_GET_MSG_FILE "GET_MSG_FILE"
#define OP_GET_MSG_TIME "GET_MSG_TIME"
#define OP_GET_MSG_CALLERID "GET_MSG_CALLERID"
#define OP_SET_MSG_MARK "SET_MSG_MARK"
#define OP_DELETE_VM "DELETE_VM"
#define OP_DELETE_VM_BY_TRUNK "DELETE_VM_BY_TRUNK"
#define OP_GENERATE_GM_FILENAME "GENERATE_GM_FILENAME"
#define OP_SAVE_GM "SAVE_GM"
#endif
...
static char *synopsis_vm_record_greeting_msg =
"Record Voicemail greeting message";

static char *descrip_vm_record_greeting_msg =
"  MailboxGreetingRecord([mailbox][@context][|options]):\n"
"  If the mailbox is specified, only that mailbox's password will be considered\n"
"valid. If the mailbox is not specified, the channel variable AUTH_MAILBOX will\n"
"be set with the authenticated mailbox.\n\n"
"  Options:\n"
...
#if defined(USE_SQLITE_DB)
static char *sysnopsis_vm_msg_data_util =
" Retreive voicemail messages data";

static char *descrip_vm_msg_data_util =
" VMDataUtils(operation|output_variable_name[|param1][|param2]):\n"
" Possible operations:\n"
" 		GET_MSG_COUNT -returns the number of  messages for\n"
" 					   specified trunk phone number (param1)\n"
"					   If param2 set, its value ('1' or '0') will be\n"
"					   matched	against 'marked' (read) criteria\n"
"		GET_NEXT_MSG - if param2 specified (current message id):\n"
"					   returns the message ID of the next message\n"
"					   relativly to current one in \n"
"					   ascending chronological order (unread first,	read"
"					   afterwards) that belongs to the specified trunk\n"
"					   (param 1)\n"
"					   else:\n"
"					   returns the message ID of the oldest message"
"					   (unread first and only if no unread- read) \n"
"					   that belongs to the specified trunk (param1) \n"
"					   If no message available - '-1' value returned \n"
"		GET_PREV_MSG - returns the message ID of the previous message \n"
"					   relativly to current one (param2) in \n"
"				       ascending chronological order that belongs to \n"
"					   the specified trunk (param 1)\n"
"					   If no message available - '-1' value returned\n"
"		GET_MSG_FILE - returns the message record file path according\n"
"					   to specified message ID"
"		SET_MSG_MARK - Set message mark to 0|1 according to message ID\n";

static char *appVMDataUtils = "VMDataUtils";
#endif
...
static char *app5 = "MailboxGreetingRecord";
...
static char db_filename[AST_MAX_FILE_NAME];
...
static void chanfmt2transfmt(struct ast_channel *chan)
{
	int code;
	static code2code_t fmt_transfmt[] = {
		{ AST_FORMAT_SLINEAR, AST_FORMAT_ALAW },
		{ -1, -1 }
	};

	if ((code = code2code(fmt_transfmt, chan->writeformat)) > -1)
	{
		ast_set_write_format(chan, code);
		ast_set_read_format(chan, code);
	}
}

static char *chanfmt2ext(struct ast_channel *chan)
{
	char *match;
	static code2str_t fmts_ext[] = {
		{ AST_FORMAT_G726, "g726-32" },
		{ AST_FORMAT_ALAW, "alaw.au" },
		{ AST_FORMAT_ULAW, "ulaw.au" },
		{ -1 }
	};

	if ((match = code2str(fmts_ext, chan->writeformat)))
	    return match;

	return ast_getformatname(chan->writeformat);
}
...
#if defined(USE_SQLITE_DB)
static int make_greeting_msg(char *dest, int len, char *dir, int num)
{
	return snprintf(dest, len, "%s/greet%04d", dir, num);
}
#endif
...
{
	voicemail_dbm_vm_message_delete_by_file(dbm, file);
...
static int get_msg_count_by_mark(int mark)
{
	vm_msg_criteria_t criteria;
	int result;

	if (!dbm)
	{
		ast_log(LOG_ERROR, "DB not accessible\n");
		return -1;
	}

	criteria.type = GET_VM_MSG_COUNT_BY_MARK;
	criteria.data.mark = mark;	

	if (voicemail_dbm_vm_message_count(dbm, &criteria, &result))
	{
		ast_log(LOG_WARNING, "Unable to get count for msgs from DB\n");		
		return -1;
	}
	return result;
}
...
static int messagecount(const char *mailbox, int *newmsgs, int *oldmsgs)
{
	*oldmsgs = get_msg_count_by_mark(1);
	*newmsgs = get_msg_count_by_mark(0);

	return !(*oldmsgs >= 0 && *newmsgs >= 0);
}

static int has_voicemail(const char *mailbox, const char *folder)
{
	return get_msg_count_by_mark(0) > 0;
}
...
static int set_remote_voicemail(const char *mailbox, int flag)
{
    char dir[256];
    char filename[256];
    FILE *file;

    /* Create the path to REMOTE mailbox: 
     * /VM_SPOOL_DIR/default/<mailbox>/REMOTE/ 
     * It's easier just to try to make it than to check for its existence. */
    make_dir(dir, sizeof(dir), "default", "", "");
    if (flag && mkdir(dir, 0700) && (errno != EEXIST))
	ast_log(LOG_WARNING, "mkdir '%s' failed: %s\n", dir, strerror(errno));
    
    make_dir(dir, sizeof(dir), "default", (char *)mailbox, "");
    if (flag && mkdir(dir, 0700) && (errno != EEXIST))
	ast_log(LOG_WARNING, "mkdir '%s' failed: %s\n", dir, strerror(errno));

    make_dir(dir, sizeof(dir), "default", (char *)mailbox, "REMOTE");
    if (flag && mkdir(dir, 0700) && (errno != EEXIST))
	ast_log(LOG_WARNING, "mkdir '%s' failed: %s\n", dir, strerror(errno));

    /* Create fake message file name */
    make_file(filename, sizeof(filename), dir, 0);
	...
	{
	    ast_log(LOG_WARNING, "Create message file %s failed: %s\n",
		filename, strerror(errno));
	    return -1;
	}
    }
...
#if defined(USE_SQLITE_DB)
	criteria.type = GET_GM_BY_GM_ID;
	criteria.data.gm_id = ext;
	
	iter = voicemail_dbm_greeting_message_iter_init(dbm, &criteria);
	if (iter && !voicemail_dbm_greeting_message_iter_get_next(iter, &gm))
		snprintf(prefile, sizeof(prefile), gm.file_path);
	else
		ast_log(LOG_DEBUG, "No greeting message for '%s'\n", ext);
	voicemail_dbm_greeting_message_iter_uninit(&iter);
...
/*	Originally outputs a file for each format in the 'format=' conf variable
 *	(and fails in trying to translate the formats).
 *	We prefer that only one file will be written, with the same format as
 *	the recording channel.
 *	OpenRG will take care of the rest.
 *
 *	ast_copy_string(fmt, vmfmts, sizeof(fmt)); */
	chanfmt2transfmt(chan);
	ast_copy_string(fmt, chanfmt2ext(chan), sizeof(fmt));
	if (!ast_strlen_zero(fmt)) {
#if defined(USE_SQLITE_DB)
		int msgnumber;

		ast_mutex_lock(&vm_dbm_lock);
		if (voicemail_dbm_file_gen_index(dbm, &msgnumber))
		{
			ast_mutex_unlock(&vm_dbm_lock);
			ast_log(LOG_ERROR, "Failed to generate voicemail message name\n");
			free_user(vmu);
			pbx_builtin_setvar_helper(chan, "VMSTATUS", "FAILED");
			return -1;
		}
		/* Get count and compare with max msgs number */
		if (voicemail_dbm_vm_message_count(dbm, NULL, &msgnum)) {
			ast_mutex_unlock(&vm_dbm_lock);
			ast_log(LOG_ERROR, "Failed to get voicemail messages count\n");
			free_user(vmu);
			pbx_builtin_setvar_helper(chan, "VMSTATUS", "FAILED");
			return -1;
		}
		...
		{
			make_file(fn, sizeof(fn), dir, msgnumber);
			strcpy(msg.file_path, fn);
			strcpy(msg.gm_id, ext);
			msg.duration = 0;
			if(chan->cid.cid_num)
				ast_copy_string(msg.callerid, chan->cid.cid_num, sizeof(msg.callerid));			
			else
				ast_copy_string(msg.callerid, "Unknown", sizeof(msg.callerid));			
			msg.time = (long)time(NULL);
			msg.mark = VM_NEW_MSG;
			strcpy(msg.trunk, chan->name);
			strcpy(msg.call_unique_id, chan->cdr->uniqueid);

			if (voicemail_dbm_vm_message_insert(dbm, &msg)) {
				ast_mutex_unlock(&vm_dbm_lock);
				ast_log(LOG_ERROR, "Saving voicemail message to db failed\n"); 
				pbx_builtin_setvar_helper(chan, "VMSTATUS", "FAILED");
				free_user(vmu);
				return -1;
			}
...
#if defined(USE_SQLITE_DB)
			voicemail_dbm_vm_message_set_duration_by_unique_id(dbm, msg.call_unique_id, duration);
			manager_event(EVENT_FLAG_CALL, "NewVoiceMailMsg",
				"VoiceMailMsgId: %d\r\n"
				"FileExtension: %s\r\n", msg.id, chanfmt2ext(chan));
#else
...
#if defined(USE_SQLITE_DB)
static int create_prefile_name(char *gm_id, int *is_new_gm, greeting_msg_t *gm)
{
	gm_criteria_t criteria;
	gm_iter_t *iter;
	...
	*is_new_gm = 0;
	criteria.type = GET_GM_BY_GM_ID;
	criteria.data.gm_id = gm_id;

	iter = voicemail_dbm_greeting_message_iter_init(dbm, &criteria);
	if (voicemail_dbm_greeting_message_iter_get_next(iter, gm))
	{
	    ast_log(LOG_DEBUG, "No greeting message for '%s'\n", gm_id);
	    *is_new_gm = 1;
	    make_greeting_msg(gm->file_path, sizeof(gm->file_path), VM_SPOOL_DIR,
			atoi(gm_id));
	    strcpy(gm->gm_id, gm_id);
	}
	voicemail_dbm_greeting_message_iter_uninit(&iter);
	return 0;
}
...
static int vm_newuser(struct ast_channel *chan, struct ast_vm_user *vmu, struct vm_state *vms, char *fmtc, signed char record_gain)
{
	...
	if (ast_test_flag(vmu, VM_FORCEGREET)) {
#if defined(USE_SQLITE_DB)
		if (create_prefile_name(vms->username, &is_new_gm, &gm))
		    return -1;

		snprintf(prefile, sizeof(prefile), "%s", gm.file_path);
#else
		...
		if (duration > 0)
		    manager_event(EVENT_FLAG_CALL, "NewGreetingMessage", "GreetingMessageId: %s\r\n", vms->username);
#if defined(USE_SQLITE_DB)
		if (duration > 0 && dbm)
		{
			gm.duration = duration;
			if (is_new_gm)
				voicemail_dbm_greeting_message_insert(dbm, &gm);
			else
				voicemail_dbm_greeting_message_set_duration(dbm, vms->username, duration);
		}
#else
...
static int vm_exec_record_greeting_msg(struct ast_channel *chan, void *data)
{
	/* XXX This is, admittedly, some pretty horrendus code.  For some
	   reason it just seemed a lot easier to do with GOTO's.  I feel
	   like I'm back in my GWBASIC days. XXX */
...
#endif
	cmd = play_record_review(chan,"vm-rec-unv",prefile, maxgreet, vmfmts, 0, vmu, &duration, NULL, record_gain);
	if ((cmd == 't') || (cmd == '#')) {
		/* Timeout */
		res = 0;
	} else if (cmd < 0) {
...
#if defined(USE_SQLITE_DB)
static int vm_data_util_get_next_msg(struct ast_channel *chan, int argc, 
	char *argv[])
{
	char buffer[20], *trunk, *dst;
	vm_msg_criteria_t msg_criteria;
	vm_msg_iter_t *msg_iter = NULL;
	vm_msg_t vm_msg_db;
	...
	dst = argv[1];
	trunk = argv[2];
	msg_criteria.data.trunk = trunk;

	if (argc == 4)
	{		 
		msg_criteria.type = GET_VM_MSG_NEXT_BY_TRUNK;
		msg_criteria.data.msg_id = atoi(argv[3]);
	}
	else
	{
		/* Get first message */
		msg_criteria.type = GET_VM_MSG_OLDEST_BY_TRUNK;
	}
	msg_iter = voicemail_dbm_vm_message_iter_init(dbm, &msg_criteria);

	if (!msg_iter)
	{
		ast_log(LOG_WARNING, "Unable to run the query\n");		
		return -1;
	}
	if (voicemail_dbm_vm_message_iter_get_next(msg_iter, &vm_msg_db))
	{
		ast_log(LOG_WARNING, "No record found for given arguments\n");
		ast_copy_string(buffer, "-1", sizeof(buffer));
		goto Exit;
	}
	snprintf(buffer, sizeof(buffer), "%d", vm_msg_db.id);

Exit:
	voicemail_dbm_vm_message_iter_uninit(&msg_iter);	
	pbx_builtin_setvar_helper(chan, dst, buffer);
	return 0;
}

static int vm_data_util_get_prev_msg(struct ast_channel *chan, int argc, 
	char *argv[])

{
	...
	trunk = argv[2];	
	msg_criteria.data.trunk = trunk;
	msg_criteria.type = GET_VM_MSG_PREV_BY_TRUNK;
	msg_criteria.data.msg_id = atoi(argv[3]);
	msg_iter = voicemail_dbm_vm_message_iter_init(dbm, &msg_criteria);
	
	if (!msg_iter)
	{
		ast_log(LOG_WARNING, "Unable to run the GET_VM_MSG_PREV_BY_TRUNK query\n");		
		return -1;
	}
...
static int vm_data_util_get_msg_count(struct ast_channel *chan, int argc, 
	char *argv[])
{
	int count;
	char buffer[20], *trunk, *dst;
	...
	if (argc == 4)
	{
		criteria.type = GET_VM_MSG_COUNT_BY_TRUNK_AND_MARK;
		criteria.data.mark = atoi(argv[3]);
	}
	else
		criteria.type = GET_VM_MSG_COUNT_BY_TRUNK;

	criteria.data.trunk = trunk;
	
	if (voicemail_dbm_vm_message_count(dbm, &criteria, &count))
	{
		ast_log(LOG_WARNING, "Unable to get count from DB\n");		
		return -1;
	}
...
static int vm_data_util_get_msg_data(struct ast_channel *chan, int argc, 
	char *argv[])
{
	vm_msg_t vm_msg_db;
	char *dst, *op_name, *value, buffer[20];
	int msg_id;
	...
	if (voicemail_common_utils_get_vm_by_id(dbm, msg_id, &vm_msg_db))
	{
		ast_log(LOG_WARNING, "Unable to get message by ID %d\n",msg_id);		
		return -1;
	}
	if (!strncasecmp(op_name, OP_GET_MSG_FILE, strlen(op_name)))
		value = vm_msg_db.file_path;
	else if(!strncasecmp(op_name, OP_GET_MSG_TIME, strlen(op_name)))
	{
		snprintf(buffer, sizeof(buffer), "%d", vm_msg_db.time);
 		value = buffer;
	}
	else if(!strncasecmp(op_name, OP_GET_MSG_CALLERID, strlen(op_name)))
		value = vm_msg_db.callerid;

	pbx_builtin_setvar_helper(chan, dst, value);
	
	return 0;
}

static int vm_data_util_set_msg_mark(struct ast_channel *chan, int argc, 
	char *argv[])
{
	int msg_id;
	msg_mark_t mark;
	...
	msg_id = atoi(argv[1]);
	mark = atoi(argv[2]);
	voicemail_dbm_vm_message_set_mark_by_id(dbm, msg_id, mark);
	manager_event(EVENT_FLAG_CALL, "VMMarked", "VMId: %d\r\n", msg_id);
	return 0;
}

static int vm_data_util_delete_vm(struct ast_channel *chan, int argc, 
	char *argv[])
{
	int msg_id;
	...
	msg_id = atoi(argv[1]);
	if (voicemail_common_utils_delete_vm_by_id(dbm, msg_id))
	{
		ast_log(LOG_WARNING, "Failed to delete msg with id %d\n", msg_id);
		return -1;
	}

	manager_event(EVENT_FLAG_CALL, "VMDeleted", "VMId: %d\r\n", msg_id);
	return 0;
}

static int vm_data_util_delete_vm_by_trunk(struct ast_channel *chan, int argc, 
	char *argv[])
{
	char *trunk;
	...
	if (voicemail_common_utils_delete_vm_by_trunk(dbm, trunk))
	{
		ast_log(LOG_WARNING, "Failed to delete messages for trunk %s\n",
			trunk);
		return -1;
	}

	manager_event(EVENT_FLAG_CALL, "VMDeleted", "TrunkId: %s\r\n", trunk);
	return 0;
}

static int vm_data_util_generate_gm_filename(struct ast_channel *chan,
	int argc, char *argv[])
{
    char *dst, *gm_id;
    greeting_msg_t gm;
	...
	if (create_prefile_name(gm_id, &is_new_gm, &gm))
		return -1;
	pbx_builtin_setvar_helper(chan, dst, gm.file_path);
	return 0;
}

static int vm_data_util_save_gm(struct ast_channel *chan, int argc, 
	char *argv[])
{
	int duration;
	char *file, *gm_id;
	...
	if (is_exist)
	{
		voicemail_dbm_greeting_message_set_duration(dbm, gm_id, duration);	
	}
	else
	{
		ast_copy_string(gm.file_path, file, sizeof(gm.file_path));
		ast_copy_string(gm.gm_id, gm_id, sizeof(gm.gm_id));
		gm.duration = duration;
		voicemail_dbm_greeting_message_insert(dbm, &gm);
	}
	manager_event(EVENT_FLAG_CALL, "NewGreetingMessage",
		"GreetingMessageId:%s\r\n", gm_id);
	return 0;
}

static int vm_exec_msg_data_util(struct ast_channel *chan, void *data)
{
	...
	if (ast_strlen_zero(data))
	{
		ast_log(LOG_WARNING, "VMDataUtil requires arguments\n");
		return -1;
	}
	...
	if (!strncasecmp(op_name, OP_GET_NEXT_MSG, strlen(op_name)))
	        rc = vm_data_util_get_next_msg(chan, argc, argv);
	else if (!strncasecmp(op_name, OP_GET_PREV_MSG, strlen(op_name)))
		rc = vm_data_util_get_prev_msg(chan, argc, argv);
	else if (!strncasecmp(op_name, OP_GET_MSG_COUNT, strlen(op_name)))
		rc = vm_data_util_get_msg_count(chan, argc, argv);
	else if (!strncasecmp(op_name, OP_GET_MSG_FILE, strlen(op_name))||
		!strncasecmp(op_name, OP_GET_MSG_TIME, strlen(op_name)) ||
		!strncasecmp(op_name, OP_GET_MSG_CALLERID, strlen(op_name)))
	{
		rc = vm_data_util_get_msg_data(chan, argc, argv);
	}
	else if (!strncasecmp(op_name, OP_SET_MSG_MARK, strlen(op_name)))
		rc = vm_data_util_set_msg_mark(chan, argc, argv);
	else if (!strncasecmp(op_name, OP_DELETE_VM, strlen(op_name)))
		rc = vm_data_util_delete_vm(chan, argc, argv);
	else if (!strncasecmp(op_name, OP_DELETE_VM_BY_TRUNK, strlen(op_name)))	
		rc = vm_data_util_delete_vm_by_trunk(chan, argc, argv);
	else if (!strncasecmp(op_name, OP_GENERATE_GM_FILENAME, strlen(op_name)))
		rc = vm_data_util_generate_gm_filename(chan, argc, argv);
	else if (!strncasecmp(op_name, OP_SAVE_GM, strlen(op_name)))
		rc = vm_data_util_save_gm(chan, argc, argv);

	if (rc)
	{
		ast_log(LOG_WARNING, "Unsupported operation '%s' or wrong "
			"number of arguments  for VMDataUtil\n", op_name);
	}
	return rc;
}
#endif
...
static int load_config(void)
{
	...
	if (cfg) {
		...
		if ((db_file = ast_variable_retrieve(cfg, "general", "db_file"))) {
			snprintf(db_filename, sizeof(db_filename), "%s/%s", VM_SPOOL_DIR, db_file);
			ast_log(LOG_DEBUG, "found Voicemail Database file: %s\n", db_filename);
		}
...
#if defined(USE_SQLITE_DB)
    res |= ast_unregister_application(appVMDataUtils);
#endif	
...
int load_module(void)
{
	...
	res |= ast_register_application(app5, vm_exec_record_greeting_msg,
		synopsis_vm_record_greeting_msg, descrip_vm_record_greeting_msg);
#if defined(USE_SQLITE_DB)
	res |= ast_register_application(appVMDataUtils, vm_exec_msg_data_util,
		sysnopsis_vm_msg_data_util, descrip_vm_msg_data_util); 
#endif
	if (res)
		return(res);
	...
	ast_install_vm_functions(has_voicemail, messagecount, set_remote_voicemail);
...
#if defined(USE_SQLITE_DB)
	if (db_filename[0] == '\0')
		snprintf(db_filename, sizeof(db_filename), "%s/%s", VM_SPOOL_DIR, "vm.db");

	dbm = voicemail_dbm_open(db_filename);
#endif
