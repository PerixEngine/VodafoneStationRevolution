...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.14.4.1.8.22 $")
...
static char template_caller[AST_CHANNEL_NAME];
...
#define HANDLE_CAUSE(cause, chan) do { \
	...
	case AST_CAUSE_UNREGISTERED: \
		...
		numnochan++; \
		break; \
	/* These cases return "NOANSWER" status */ \
...
#define HANGUP_CHANNEL(in, o, bridged_to, cause) do { \
	if (bridged_to && bridged_to == o->chan) \
	{ \
		ast_channel_post_bridge(in, bridged_to); \
		bridged_to = NULL; \
	} \
	ast_hangup(o->chan); \
...
static struct ast_channel *wait_for_answer(struct ast_channel *in, struct localuser *outgoing, int *to, struct ast_flags *peerflags, int *sentringing, char *status, size_t statussize, int busystart, int nochanstart, int congestionstart, int unallocstart, int priority_jump, int *result)
{
	struct localuser *o;
	int found;
	int numlines;
	...
	int numunallocated = unallocstart;
	int prestart = busystart + congestionstart + nochanstart + unallocstart;
	int cause;
	int orig = *to;
	struct ast_frame *f;
	...
	struct ast_channel *bridged_to = NULL;
	...
	while (*to && !peer) {
		...
		if (found < 0) {
			if (numlines == (numbusy + numcongestion + numnochan + numunallocated)) {
				if (option_verbose > 2)
					ast_verbose( VERBOSE_PREFIX_2 "Everyone is busy/congested at this time (%d:%d/%d/%d/%d)\n", numlines, numbusy, numcongestion, numnochan, numunallocated);
				if (numbusy)
					strcpy(status, "BUSY");	
				else if (numcongestion)
					strcpy(status, "CONGESTION");
				else if (numunallocated)
				...
				if (!ast_strlen_zero(o->chan->call_forward)) {
					...
					if (!o->chan) {
						...
						if (ast_call(o->chan, tmpchan, 0)) {
							...
							HANGUP_CHANNEL(in, o, bridged_to, AST_CAUSE_FAILURE);
						} else {
				...
				if (f) {
					if (f->frametype == AST_FRAME_CONTROL) {
						...
						case AST_CONTROL_ANSWER:
							...
							in->answer_data = o->chan->answer_data;
							break;
						case AST_CONTROL_BUSY:
							...
							if (f->data)
								pbx_builtin_setvar_helper(in, "BUSYSTATE", (char *)f->data);
							HANGUP_CHANNEL(in, o, bridged_to, AST_CAUSE_BUSY);
							break;
						case AST_CONTROL_UNALLOCATED:
							if (option_verbose > 2)
								ast_verbose( VERBOSE_PREFIX_3 "%s is unallocated\n", o->chan->name);
							in->hangupcause = o->chan->hangupcause;
							HANGUP_CHANNEL(in, o, bridged_to, AST_CAUSE_UNALLOCATED);
							break;
						case AST_CONTROL_DECLINE:
							in->hangupcause = o->chan->hangupcause;
							strcpy(status, "CANCEL");
							peer = NULL; /* cause hangup_tree() */
							goto exit;
						case AST_CONTROL_CONGESTION:
							if (option_verbose > 2)
								ast_verbose( VERBOSE_PREFIX_3 "%s is circuit-busy\n", o->chan->name);
							in->hangupcause = o->chan->hangupcause;
							HANGUP_CHANNEL(in, o, bridged_to, AST_CAUSE_CONGESTION);
							break;
						...
						case AST_CONTROL_PROGRESS:
							if (option_verbose > 2)
 							...
 							/* Allow RTP data exchange before call was answered */
							ast_channel_make_compatible(in, o->chan);
 							ast_channel_pre_bridge(in, o->chan);
 							bridged_to = o->chan;
							if (!ast_test_flag(outgoing, OPT_RINGBACK))
								ast_indicate_data(in, AST_CONTROL_PROGRESS, f->data, f->datalen);
							break;
						...
						case AST_CONTROL_TRANSPARENT_DATA:
							if (option_verbose > 2)
								ast_verbose(VERBOSE_PREFIX_3 "Call on %s is transfering transparent data\n", o->chan->name);
							ast_indicate_data(in, AST_CONTROL_TRANSPARENT_DATA, f->data, f->datalen);
							break;
						case -1:
					...
					in->hangupcause = o->chan->hangupcause;
					HANGUP_CHANNEL(in, o, bridged_to, in->hangupcause);
				}
			}
			o = o->next;
		}
		if (winner == in) {
			...
			if (single && (f->frametype == AST_FRAME_CONTROL) && 
				(f->subclass == AST_CONTROL_FAX || f->subclass == AST_CONTROL_T38 || f->subclass == AST_CONTROL_MODEM)) {
				ast_log(LOG_DEBUG, "%s requested a %s call, passing it to %s\n", in->name,
					f->subclass == AST_CONTROL_FAX ? "fax" : f->subclass == AST_CONTROL_MODEM ? "modem" : "T.38", outgoing->chan->name);
				ast_indicate(outgoing->chan, f->subclass);
			}
			ast_frfree(f);
		}
...
exit:
	/* Close the bridge that was opened for RTP progress */
	if (bridged_to)
		ast_channel_post_bridge(in, bridged_to);
...
static int set_template_vars(struct ast_channel *chan, char *caller)
{
	if (ast_strlen_zero(caller))
	    return 0;

	chan->is_template_trunk_extension = 1;

	template_count++;
	pbx_builtin_setvar_helper(NULL, "CURR_CALL_STR_DAY", caller);
	pbx_builtin_setvar_helper(NULL, "CURR_CALL_STR_NIGHT", caller);
	strcpy(template_caller, caller);
...
static int dial_exec_full(struct ast_channel *chan, void *data, struct ast_flags *peerflags)
{
	...
	/* If caller is different from template_caller limit the call */
	if (!ast_strlen_zero(template_caller) &&
	    !ast_strlen_zero(chan->template_caller_name) &&
	    strcmp(template_caller, chan->template_caller_name))
	{
	    strcpy(status, "CALL_LIMIT");
	    chan->hangupcause = AST_CAUSE_CALL_LIMIT;
	    goto out;
	}

	set_template_vars(chan, chan->template_caller_name);
	...
	do {
		...
		*number = '\0';
		...
		chan->call_type = tmp->chan->tech->call_description;
		tmp->chan->call_type = chan->tech->call_description;
		...
		strcpy(tmp->chan->shared_callid, chan->shared_callid);
	...
	time(&start_time);
	peer = wait_for_answer(chan, outgoing, &to, peerflags, &sentringing, status, sizeof(status), numbusy, numnochan, numcongestion, numunallocated, ast_test_flag(&opts, OPT_PRIORITY_JUMP), &result);
	...
	if (peer) {
		...
		set_template_vars(chan, peer->template_caller_name);
			...
			if (!res) {
				if ((macro_result = pbx_builtin_getvar_helper(peer, "MACRO_RESULT"))) {
					if (!strcasecmp(macro_result, "BUSY")) {
					    ...
					    || !strcasecmp(macro_result, "UNALLOCATED")) {
						ast_copy_string(status, macro_result, sizeof(status));
						ast_set_flag(peerflags, OPT_GO_ON);	
						res = -1;
					}
...
out:
	...
	if (strcmp(status, "CALL_LIMIT") && template_count && chan->is_template_trunk_extension) {
	    	template_count--;
		if (!template_count) {
			pbx_builtin_setvar_helper(NULL, "CURR_CALL_STR_DAY", NULL);
			pbx_builtin_setvar_helper(NULL, "CURR_CALL_STR_NIGHT", NULL);
			template_caller[0] = '\0';
		}
	}
