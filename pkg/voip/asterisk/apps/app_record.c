...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.6.6.3 $")
...
static int record_exec(struct ast_channel *chan, void *data)
{
	...
	int count = 0;
	int percentflag = 0;
	char *filename, *ext = NULL, *silstr, *maxstr, *options, *duration_out;
	char *vdata, *p;
	int i = 0;
	char tmp[256];
	...
	maxstr = strsep(&p, "|");	
	options = strsep(&p, "|");
	duration_out = strsep(&p, "|");

	if (filename) {
		if (strstr(filename, "%d"))
			percentflag = 1;
		/* Support the format with '.',as opposite to original asterisk
		 * implementation
		 */
		ext = strchr(filename, '.'); 
		if (!ext)
 ...
 out:
	...
	if (duration_out)	
		pbx_builtin_set_int_var_helper(chan, duration_out, duration);	
