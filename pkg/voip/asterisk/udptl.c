...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.3.4.8 $")
...
#define DEFAULT_MAX_BITRATE 14400
...
struct ast_udptl {
	...
	/*! this option indicates the maximum bit rate to use */
	int max_bitrate;

	/*! T38 Asn.1 version. Either 0 or 1 */
	int t38_version;

	/*! Facsimile image conversion options */
	int conversion_options;

	/*! Rate management method. Either TRANS_TCF or LOC_TCF */
...
#endif
	udptl->f[0].frametype = AST_FRAME_MODEM;
	udptl->f[0].subclass = AST_MODEM_T38;
	udptl->f[0].datalen = res;
	udptl->f[0].data = udptl->rawdata + AST_FRIENDLY_OFFSET;
	udptl->f[0].offset = 0;
	udptl->f[0].src = "UDPTL";
	udptl->f[0].prev = NULL;
	udptl->f[0].next = NULL;
...
{
	if (udptl)
	    return udptl->t38_version;
    else 
	{
	    ast_log(LOG_WARNING, "udptl structure is null\n");
	    return 0; /* Default value according to RFC 4612 */
    }	
}
...
{
    if (udptl)
	    udptl->t38_version = version;
    else
	    ast_log(LOG_WARNING, "udptl structure is null\n");
}

int ast_udptl_get_conversion_option(
	struct ast_udptl* udptl)
{
	if (udptl)
	    return udptl->conversion_options;
    else 
	{
	    ast_log(LOG_WARNING, "udptl structure is null\n");
	    return UDPTL_CONVERSION_NONE;
    }	
}
...
{
    if (udptl)
	    udptl->conversion_options |= conversion_option;
    else
	    ast_log(LOG_WARNING, "udptl structure is null\n");
}
...
{
	if (udptl)
	    return udptl->rate_management_method;
    else 
	{
	    ast_log(LOG_WARNING, "udptl structure is null\n");
	    return UDPTL_TRANS_TCF;
    }	
}
...
{
    if (udptl)
	    udptl->rate_management_method = rate_management_method;
    else
	    ast_log(LOG_WARNING, "udptl structure is null\n");
}
...
struct ast_udptl *ast_udptl_new_with_bindaddr(struct sched_context *sched, struct io_context *io, int callbackmode, struct in_addr addr)
{
	...
	udptl->t38_version = 0;
	udptl->conversion_options = UDPTL_CONVERSION_NONE;
	udptl->rate_management_method = UDPTL_TRANS_TCF;
	udptl->max_bitrate = DEFAULT_MAX_BITRATE;
...
int ast_udptl_set_so_mark(struct ast_udptl *udptl, int so_mark)
{
	...
	if ((res = setsockopt(udptl->fd, SOL_SOCKET, SO_MARK, &so_mark, 
	    sizeof(so_mark)))) 
	{
	    ast_log(LOG_WARNING, "UDPTL unable to set SO_MARK to %d\n",
		so_mark);
	}
	return res;
#else
...
void ast_udptl_reload(void)
{
	...
	if ((cfg = ast_config_load("udptl.conf"))) {
		...
		if ((s = ast_variable_retrieve(cfg, "general", "T38FaxUdpEC"))) {
			ast_log(LOG_DEBUG, "UDPTL EC mode is %s\n", s);
		...
		if ((s = ast_variable_retrieve(cfg, "general", "T38FaxMaxDatagram"))) {
			...
			ast_log(LOG_DEBUG, "UDPTL Max Datagram is %d\n", udptlmaxdatagram);
		...
		if ((s = ast_variable_retrieve(cfg, "general", "UDPTLFECentries"))) {
			...
			ast_log(LOG_DEBUG, "UDPTL FEC entries is %d\n", udptlfecentries);
		...
		if ((s = ast_variable_retrieve(cfg, "general", "UDPTLFECspan"))) {
			udptlfecspan = atoi(s);
			ast_log(LOG_DEBUG, "UDPTL FEC span is %d\n", udptlfecspan);
			if (udptlfecspan < 0)
				udptlfecspan = 0;
