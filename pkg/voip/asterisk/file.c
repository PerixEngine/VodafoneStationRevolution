...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.17.4.2 $")
...
int ast_filestream_get_format(struct ast_filestream *fs)
{
	return fs->fmt->format;
}

/* Calculate when is the next scheduled expiration. We need to compensate if we
 * are already late. */
static void calc_whennext(struct ast_filestream *s, int *whennext_samp)
{
    struct timeval now, compensation = ast_tv(0, 0);
    struct timeval whennext = ast_samp2tv(*whennext_samp, 8000);

    now = ast_tvfromboot();

    if (ast_tvcmp(now, s->timeexpected) > 0)
	compensation = ast_tvsub(now, s->timeexpected);

    /* Calculate the time of next expected packet. */
    s->timeexpected = ast_tvadd(ast_tvzero(s->timeexpected) ?
	now : s->timeexpected, whennext);

    /* If compensation is about to be bigger than delay between samples, then
     * schedule to 1ms from now. */
    if (ast_tvcmp(compensation, whennext) > 0)
	whennext = ast_tv(0, 1000);
    else
	whennext = ast_tvsub(whennext, compensation);

    *whennext_samp = ast_tvdiff_ms(whennext, ast_tv(0, 0))*8;
}
