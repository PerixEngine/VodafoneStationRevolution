...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.9.14.7 $")
...
static int pbx_builtin_noanswer(struct ast_channel *, void *);
static int pbx_builtin_calllimit(struct ast_channel *, void *);
static int pbx_builtin_unallocated(struct ast_channel *, void *);
...
} builtins[] = 
{
	...
	{ "Noanswer", pbx_builtin_noanswer,
	"Indicate the No Answer condition",
	"  Noanswer: This application will indicate the \"no answer\"\n"
	"condition to the calling channel."
	},

	{ "Calllimit", pbx_builtin_calllimit,
	"Indicate the Call limit condition",
	"  Calllimit: This application will indicate the \"call limit\"\n"
	"condition to the calling channel."
	},

	{ "Unallocated", pbx_builtin_unallocated,
	"Indicate the Unallocated number condition",
	"  Unallocated([timeout]): This application will indicate the unallocated number\n"
	"condition to the calling channel. If the optional timeout is specified, the\n"
	"calling channel will be hung up after the specified number of seconds.\n"
	"Otherwise, this application will wait until the calling channel hangs up.\n"
	},
		...
		if (!ast_exists_extension(c, c->context, c->exten, 1, c->cid.cid_num)) {
			...
			if (waittime) {
				...
				if (!strcasecmp(status, "CONGESTION"))
					...
					res = pbx_builtin_congestion(c, "10");
				else if (!strcasecmp(status, "UNALLOCATED"))
					res = pbx_builtin_unallocated(c, "10");
...
int ast_check_timing(struct ast_timing *i)
{
	...
	if (!(i->minmask[tm.tm_hour * 2 + (tm.tm_min >= 30 ? 1 : 0)] & 
		(1 << (tm.tm_min >= 30 ? tm.tm_min - 30 : tm.tm_min))))
		return 0;
...
/* The following message is not so important but very frequent.
 * Only display it when debug/verbose level is very high.
 */
#define LOG do { 	if (option_debug > 5) {\
		if (tmp->matchcid) { \
			ast_log(LOG_DEBUG, "Added extension '%s' priority %d (CID match '%s') to %s\n", tmp->exten, tmp->priority, tmp->cidmatch, con->name); \
		} else { \
			ast_log(LOG_DEBUG, "Added extension '%s' priority %d to %s\n", tmp->exten, tmp->priority, con->name); \
		} \
	} else if (option_verbose > 11) { \
		if (tmp->matchcid) { \
...
int ast_pbx_outgoing_exten(const char *type, const struct ast_codec_pref *formats, void *data, int timeout, const char *context, const char *exten, int priority, int *reason, int sync, const char *cid_num, const char *cid_name, struct ast_variable *vars, struct ast_channel **channel)
{
	struct ast_channel *chan;
...
int ast_pbx_outgoing_app(const char *type, const struct ast_codec_pref *formats, void *data, int timeout, const char *app, const char *appdata, int *reason, int sync, const char *cid_num, const char *cid_name, struct ast_variable *vars, struct ast_channel **locked_channel)
{
	struct ast_channel *chan;
	struct async_stat *as;
	struct app_tmp *tmp;
...
{
    	char *state = pbx_builtin_getvar_helper(chan, "BUSYSTATE");

	ast_indicate_data(chan, AST_CONTROL_BUSY, state, 
...
static int pbx_builtin_noanswer(struct ast_channel *chan, void *data)
{
	ast_indicate(chan, AST_CONTROL_NOANSWER);
	ast_setstate(chan, AST_STATE_BUSY);
	wait_for_hangup(chan, data);
	return -1;
}

static int pbx_builtin_calllimit(struct ast_channel *chan, void *data)
{
	ast_indicate(chan, AST_CONTROL_CALL_LIMIT);
	ast_setstate(chan, AST_STATE_BUSY);
	wait_for_hangup(chan, data);
	return -1;
}
 ...
 * \ingroup applications
 */
static int pbx_builtin_unallocated(struct ast_channel *chan, void *data)
{
	ast_indicate(chan, AST_CONTROL_UNALLOCATED);
	ast_setstate(chan, AST_STATE_BUSY);
	wait_for_hangup(chan, data);
	return -1;
}
...
void pbx_builtin_set_int_var_helper(struct ast_channel *chan, const char *name,
	const int value)
{
	char buffer[20];

	snprintf(buffer, sizeof(buffer), "%d", value);
	pbx_builtin_setvar_helper(chan, name, buffer);
}
