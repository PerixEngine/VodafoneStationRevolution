...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.6.4.1 $")
...
"Usage: reload extensions [--changed-only]\n"
"Usage: reload extensions.conf without reloading any other modules\n"
"       If --changed-only flag is used, extensions.conf is reloaded only if\n"
...
"         extensions reload --changed-only\n";
...
static int handle_reload_extensions(int fd, int argc, char *argv[])
{
	...
	if (argc == 3 && !strcmp("--changed-only", argv[2]))
		check_conf_file = 1;
