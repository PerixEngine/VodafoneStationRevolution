...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.7.2.2.4.3 $")
...
void ast_cdr_getvar(struct ast_cdr *cdr, const char *name, char **ret, char *workspace, int workspacelen, int recur) 
{
	...
	*ret = NULL;
	...
	else if (!strcasecmp(name, "hangupcause"))
		snprintf(workspace, workspacelen, "%d", cdr->hangupcause);
...
void ast_cdr_hangupcause(struct ast_cdr *cdr, int cause)
{
	char *chan;
...
int ast_cdr_update(struct ast_channel *c)
{
	...
	if (!ast_test_flag(cdr, AST_CDR_FLAG_LOCKED) && !cdr->next)
	{
	    num = c->cid.cid_ani ? c->cid.cid_ani : c->cid.cid_num;
...
static int handle_cli_show_cdr(int fd, int argc, char *argv[])
{
    struct ast_channel *chan;
    struct ast_cdr *cdr;
    char us_buf[INET_ADDRSTRLEN] = "";
    char them_buf[INET_ADDRSTRLEN] = "";
    ...
    if (!chan->cdr)
    {
        chan = ast_bridged_channel(chan);
	if (!chan || !chan->cdr)
	    return 0;
    }
    cdr = chan->cdr;

    ast_inet_ntoa(us_buf, sizeof(us_buf), cdr->us.sin_addr);
    ast_inet_ntoa(them_buf, sizeof(them_buf), cdr->them.sin_addr);

    ast_cli(fd, "Channel: %s\n", cdr->channel);
    ast_cli(fd, "DestChannel: %s\n", cdr->dstchannel);
    ast_cli(fd, "Our IP: %s\n", us_buf);
    ast_cli(fd, "Peer IP: %s\n", them_buf);
    ast_cli(fd, "Caller number: %s\n", cdr->src);
    ast_cli(fd, "Called number: %s\n", cdr->dst);
    ast_cli(fd, "StartTime: %ld\n", cdr->start.tv_sec);
    ast_cli(fd, "AnswerTime: %ld\n", cdr->answer.tv_sec);
    ast_cli(fd, "PacketsSent: %d\n", cdr->tx_packets);
    ast_cli(fd, "PacketsReceived: %d\n", cdr->rx_packets);
    ast_cli(fd, "PacketsLost: %d\n", cdr->lost_packets);
...
static struct ast_cli_entry cli_show_cdr = {
	.cmda = { "cdr", "show", NULL },
	.handler = handle_cli_show_cdr,
	.summary = "Display a CDR of an active channel",
	.usage =
	"Usage: cdr show <channel>\n"
	"	Displays the Call Detail Record of the channel.\n"
};
