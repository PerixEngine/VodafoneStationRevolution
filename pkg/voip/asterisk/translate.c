...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.9.8.3 $")
...
void ast_translator_free_path(struct ast_trans_pvt *p)
{
	...
	while(pn) {
		pl = pn;
		pn = pn->next;
		if (pl->state && pl->step && pl->step->destroy)
...
static void calc_cost(struct ast_translator *t, int samples)
{
	...
	cost = ast_tvdiff_ms(ast_tvfromboot(), start);
	t->destroy(pvt);
	t->cost = cost / samples;
	if (!t->cost)
		t->cost = 1;
...
int ast_translator_best_choice(int *dst, int *srcs, int *cost)
{
	int x,y;
	int best = -1;
	int bestdst = 0;
	...
	for (x = 0; x < MAX_FORMAT; ++x)
	{
		if ((1 << x) & formats)
		{
			for (y = 0; y < MAX_FORMAT; ++y)
			{
				...
				{
					if (max_transcoding_cost && tr_matrix[x][y].cost > max_transcoding_cost)
						continue;
