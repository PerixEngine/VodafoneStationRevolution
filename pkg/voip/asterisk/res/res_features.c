	...
	if ((chan = ast_request(type, formats, data, &cause))) {
		...
		if (!ast_call(chan, data, timeout)) {
			...
			while (!ast_check_hangup(caller) && timeout && (chan->_state != AST_STATE_UP)) {
				...
				if(ast_tvdiff_ms(ast_tvfromboot(), started) > timeout) {
...
int ast_bridge_call(struct ast_channel *chan,struct ast_channel *peer,struct ast_bridge_config *config)
{
	...
	for (;;) {
		...
		if (config->feature_timer) {
			...
			diff = ast_tvdiff_ms(ast_tvfromboot(), start);
...
static void *do_parking_thread(void *ignore)
{
	...
	for (;;) {
		...
		while(pu) {
			...
			tms = ast_tvdiff_ms(ast_tvfromboot(), pu->start);
