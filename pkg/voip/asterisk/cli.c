...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.9.6.1.2.3 $")
...
static int handle_reload(int fd, int argc, char *argv[])
{
	int x = 1;
	int res;
	int check_conf_file = 0;
	if (argc < 1)
		return RESULT_SHOWUSAGE;
	if (argc > 1 && !strcmp(argv[1], "--changed-only"))
	{
		check_conf_file = 1;
		x++;
	}
	if (argc > x) { 
		for (;x<argc;x++) {
			res = ast_module_reload(argv[x], check_conf_file);
			switch(res) {
			case 0:
		...
		ast_module_reload(NULL, check_conf_file);
	return RESULT_SUCCESS;
}
...
#define VERBOSE_FORMAT_STRING2 "%-20.20s %-20.20s %-16.16s %-4.4s %-7.7s %-12.12s %-25.25s %-15.15s %8.8s %-11.11s %-20.20s\n"
	...
	while ((c = ast_channel_walk_locked(c)) != NULL) {
		...
		if ((concise || verbose)  && c->cdr && !ast_tvzero(c->cdr->start)) {
			duration = (int)(ast_tvdiff_ms(ast_tvfromboot(), c->cdr->start) / 1000);
			if (verbose) {
				durh = duration / 3600;
		...
		if (concise) {
			ast_cli(fd, CONCISE_FORMAT_STRING, c->name, c->context, c->exten, c->priority, ast_state2str(c->_state, ast_test_flag(c, AST_FLAG_CALL_ONHOLD)),
			...
			ast_cli(fd, VERBOSE_FORMAT_STRING, c->name, c->context, c->exten, c->priority, ast_state2str(c->_state, ast_test_flag(c, AST_FLAG_CALL_ONHOLD)),
			...
			ast_cli(fd, FORMAT_STRING, c->name, locbuf, ast_state2str(c->_state, ast_test_flag(c, AST_FLAG_CALL_ONHOLD)), appdata);
		}
		numchans++;
...
static int handle_showchan(int fd, int argc, char *argv[])
{
	...
	ast_cli(fd, 
		...
		(c->cid.cid_dnid ? c->cid.cid_dnid : "(N/A)" ), ast_state2str(c->_state, ast_test_flag(c, AST_FLAG_CALL_ONHOLD)), c->_state, c->rings, ast_codec_pref_bits(&c->nativeformats), c->writeformat, c->readformat,
