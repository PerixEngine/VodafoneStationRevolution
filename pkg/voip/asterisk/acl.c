...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.5.10.4 $")
...
int ast_apply_ha_default(struct ast_ha *ha, struct sockaddr_in *sin,
    int def_sense)
{
	int res = def_sense;
	while (ha) {
...
{
	/* Start optimistic */
	return ast_apply_ha_default(ha, sin, AST_SENSE_ALLOW);
}
	...
	struct sockaddr_in *sin, const char *service, int min_srv_ttl)
{
	...
	if ((ret = ast_srv_lookup(context, service, min_srv_ttl, &thost, &tport,
		&ttl)))
	{
		ast_log(LOG_WARNING, "Can't get valid SRV entry\n");
		return ret;
	}

	/* Resolve IP address from found host */
	if (!(hp = ast_gethostbyname(thost, &ahp)))
	{
		ast_log(LOG_WARNING, "Cant resolve host %s\n", thost);
		return -1;
	}
