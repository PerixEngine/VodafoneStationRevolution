...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.10.6.5 $")
...
{
    return ast_app_getdata_allow_escape(c, prompt, s, maxlen, timeout, NULL, 0, NULL);
}

int ast_app_getdata_allow_escape(struct ast_channel *c, char *prompt, char *s, int maxlen, int timeout, char *escape_code, int escape_timeout, char *break_keys)
{
	int res=0,to,fto;
        char *front=NULL,*back=NULL,*full_s=s;
	...
	if (prompt) {
		...
		while (!res && front) {
			...
			if (!res) {
				res = ast_waitstream(c, AST_DIGIT_ANY);
				ast_stopstream(c);
				front = back;
				continue;
			}
	...
	if (res > 0) {
		*s++ = res;
		maxlen--;
		if (break_keys && strchr(break_keys, res))
		{ 
			*s = '\0';
			return 0;
		}	
	}
	res = ast_readstring_allow_escape(c, s, maxlen, to, fto, "#", full_s, escape_code, escape_timeout, break_keys);
	return res;
}
...
static int (*ast_set_remote_vm_func)(const char *mailbox, int flag) = NULL;

void ast_install_vm_functions(int (*has_voicemail_func)(const char *mailbox, const char *folder),
			      int (*messagecount_func)(const char *mailbox, int *newmsgs, int *oldmsgs),
			      int (*set_remote_vm_func)(const char *mailbox, int flag))
{
	ast_has_voicemail_func = has_voicemail_func;
	ast_messagecount_func = messagecount_func;
	ast_set_remote_vm_func = set_remote_vm_func;
}
...
void ast_uninstall_vm_functions(void)
{
	...
	ast_set_remote_vm_func = NULL;
}

int ast_app_set_remote_vm(const char *mailbox, int flag)
{
	static int warned = 0;
	if (ast_set_remote_vm_func)
		return ast_set_remote_vm_func(mailbox, flag);

	if ((option_verbose > 2) && !warned) {
		ast_verbose(VERBOSE_PREFIX_3 "Remote message set requested for mailbox %s but voicemail not loaded.\n", mailbox);
		warned++;
	}
	return 0;
}
...
static void ast_write_silence_to_fs(struct ast_filestream *fs, int format,
	int ms)
{
	char silence_buf[320] = { };
	/* Each silence frame has the length of 20ms. */
	struct ast_frame silence_frame_slinear = {
	...
	struct ast_frame silence_frame_g729 = {
		.frametype = AST_FRAME_VOICE,
		.subclass = AST_FORMAT_G729A,
		.data = silence_buf,
		.samples = 160,
		.datalen = 2,
	};
	...
	if (ms % 20)
	{
		ast_log(LOG_WARNING, "ast_write_silence_to_fs() works in multiples of "
			"20 ms.");
	}

	if (format == AST_FORMAT_G729A)
		silence_frame = &silence_frame_g729;
	else
		silence_frame = &silence_frame_slinear;

	frames = ms / 20;

	while (frames--)
		ast_writestream(fs, silence_frame);
}
...
int ast_play_and_record(struct ast_channel *chan, const char *playfile, const char *recordfile, int maxtime, const char *fmt, int *duration, int silencethreshold, int maxsilence, const char *path)
{
	...
	unsigned int silence_ms, ts, old_ts, d_ts;
	   ...
	   we read a # or get a hangup */
		...
		for(;;) {
			...
			if (!f)
				break;
#endif
			/* Here we are dealing with a situation in which the other party
			 * mutes the call and thus stops sending voice frames.
			 * Three situations are handled:
			 * 1) The call starts with the other party muted and not sending
			 * voice frames. In such a situation, the time passing without
			 * frames is accumulated, and when the first voice frame arrives
			 * or the call ends, a matching number of silence frames will be
			 * written to the vm file.
			 * 2) The other side is muted in the middle of a call.
			 * Once we get the next voice frame (after unmute), we use its
			 * timestamp to determine the duration of silence, and write
			 * the appropriate number of silence frames.
			 * 3) The call hangs up during mute.
			 * The time accumulated since the start of the conversation or
			 * the last voice frame determines the number of silence frames
			 * to be written. */
			res = ast_waitfor(chan, 500);
			if (!res)
			{
				f = &null_frame;
				silence_ms += 500;
			...
			else
			{
				...
				if (!old_ts)
				{
					...
					for (x=0;x<fmtcnt;x++)
					{
						ast_write_silence_to_fs(others[x], 
							ast_filestream_get_format(others[x]), silence_ms);
					}
				}
				else
				{
					...
					if (d_ts > 160)
					{
						for (x=0;x<fmtcnt;x++) {
							ast_write_silence_to_fs(others[x], 
								ast_filestream_get_format(others[x]),
								(d_ts / 160) * 20);
						}
					}
				}
	...
	*duration = MIN(end - start, maxtime);
...
enum AST_LOCK_RESULT ast_lock_path(const char *path)
{
	...
	time(&start);
	while (((fd = open(s, O_WRONLY | O_CREAT | O_EXCL, 0600)) < 0) && (errno == EEXIST) && (time(NULL) - start < 5))
		usleep(1);
	if (fd < 0) {
		if (errno == EEXIST) {
