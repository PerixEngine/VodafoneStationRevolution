...
*/
	...
	if( ast_mutex_trylock(&p->record_lock) == 0 ) {
		...
		cid_record_time = ast_tvfromboot();
		if (option_verbose>3) 
		...
		if (option_verbose>3) 
			ast_verbose(VERBOSE_PREFIX_4 "CID record - skipped %ldms trailing ring\n",
				 ast_tvdiff_ms(ast_tvfromboot(), cid_record_time));
		cid_record_time = ast_tvfromboot();
	...
	switch (e->type) {
		...
		case VPB_TONEDETECT:
			if (e->data == VPB_BUSY || e->data == VPB_BUSY_308 || e->data == VPB_BUSY_AUST ) {
				...
				if ( ast_tvdiff_ms(ast_tvfromboot(), p->lastgrunt) > gruntdetect_timeout ) {
...
/*	ast_log(LOG_DEBUG, "%s: vpb_write: Checked frame type..\n", p->dev); */
	...
	tdiff = ast_tvdiff_ms(ast_tvfromboot(), p->lastplay);
...
/*	ast_log(LOG_DEBUG, "%s: vpb_write: play_buf_time %d\n", p->dev, p->play_buf_time); */
	...
	if ((p->read_state == 1)&&(p->play_buf_time<5)){
		play_buf_time_start = ast_tvfromboot();
/*		res = vpb_play_buf_sync(p->handle, (char*)frame->data, tdiff*8*2); */
		...
		p->play_buf_time = ast_tvdiff_ms(ast_tvfromboot(), play_buf_time_start);
	}
	else {
		p->chuck_count++;
...
static struct ast_channel *vpb_new(struct vpb_pvt *me, int state, char *context)
{
	...
	if (tmp) {
		...
		me->lastgrunt  = ast_tvfromboot(); /* Assume at least one grunt tone seen now. */
		me->lastplay  = ast_tvfromboot(); /* Assume at least one grunt tone seen now. */
