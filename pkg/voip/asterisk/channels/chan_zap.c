...
#include <sys/ioctl.h>
#ifdef __linux__
#include <voip/zaptel/zaptel.h>
#else
#include <zaptel.h>
#endif /* __linux__ */
#include <math.h>
#include <voip/zaptel/tonezone.h>
#include <ctype.h>
#ifdef ZAPATA_PRI
#include <libpri.h>
...
static int zt_answer(struct ast_channel *ast)
{
	...
	case SIG_FXOKS:
		...
		if(p->hanguponpolarityswitch) {
			p->polaritydelaytv = ast_tvfromboot();
		}
	...
	switch(res) {
		...
		case ZT_EVENT_POLARITY:
			...
			if(p->hanguponpolarityswitch &&
				...
				ast_log(LOG_DEBUG, "Polarity Reversal event occured - DEBUG 1: channel %d, state %d, pol= %d, aonp= %d, honp= %d, pdelay= %d, tv= %d\n", p->channel, ast->_state, p->polarity, p->answeronpolarityswitch, p->hanguponpolarityswitch, p->polarityonanswerdelay, ast_tvdiff_ms(ast_tvfromboot(), p->polaritydelaytv) );
			
				if(ast_tvdiff_ms(ast_tvfromboot(), p->polaritydelaytv) > p->polarityonanswerdelay) {
	...
	for(;;) {
		...
		if (doidling && pri_is_up(pri)) {
			...
			printf("nextidle: %d, haveidles: %d, ms: %ld, minunsed: %d\n",
				nextidle, haveidles, ast_tvdiff_ms(ast_tvfromboot(), lastidle), minunused);
#endif
			if (nextidle > -1) {
				if (ast_tvdiff_ms(ast_tvfromboot(), lastidle) > 1000) {
		...
		for (i=0; i<NUM_DCHANS; i++) {
			...
			if ((next = pri_schedule_next(pri->dchans[i]))) {
				...
				tv = ast_tvsub(*next, ast_tvfromboot());
				if (tv.tv_sec < 0) {
					tv = ast_tv(0,0);
				}
