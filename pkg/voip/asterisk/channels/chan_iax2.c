...
retry:
	ast_mutex_lock(&iaxsl[callno]);
	pvt = iaxs[callno];
	lastused[callno] = ast_tvfromboot();
...
static void update_jbsched(struct chan_iax2_pvt *pvt) {
    ...
    when = ast_tvdiff_ms(ast_tvfromboot(), pvt->rxcore);
...
static unsigned int fix_peerts(struct timeval *tv, int callno, unsigned int ts)
{
	...
	if (ast_tvzero(iaxs[callno]->rxcore)) {
		...
		iaxs[callno]->rxcore = ast_tvfromboot();
...
static unsigned int calc_timestamp(struct chan_iax2_pvt *p, unsigned int ts, struct ast_frame *f)
{
	...
	*/
	...
	if (delivery && !ast_tvzero(*delivery)) {
		...
		ms = ast_tvdiff_ms(ast_tvfromboot(), p->offset);
		if (ms < 0)
			ms = 0;
		if (voice) {
...
static unsigned int calc_rxstamp(struct chan_iax2_pvt *p, unsigned int offset)
{
	...
	ms = ast_tvdiff_ms(ast_tvfromboot(), p->rxcore);
#ifdef IAXTESTS
	if (test_jit) {
