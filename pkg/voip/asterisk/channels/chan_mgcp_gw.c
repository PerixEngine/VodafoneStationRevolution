...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.4.2.1 $")
...
static const char type[] = "MGCP_GW";
static const char tdesc[] = "Media Gateway Control Protocol (MGCP)";
static const char config[] = "mgcp_gw.conf";
...
static struct mgcp_gateway {
	...
	char name[80];
	char domain[80];
	int isdomaindottedip; /* SC: is the name FQDN or dotted ip */
...
static int transmit_notify(struct mgcp_subchannel *sub, char *event);
...
static int mgcp_show_endpoints(int fd, int argc, char *argv[])
{
	...
	while(g) {
		e = g->endpoints;
		ast_cli(fd, "Gateway '%s' at %s\n", g->name, g->addr.sin_addr.s_addr ? ast_inet_ntoa(iabuf, sizeof(iabuf), g->addr.sin_addr) : ast_inet_ntoa(iabuf, sizeof(iabuf), g->defaddr.sin_addr));
		while(e) {
	...
	if (tmp) {
		...
		if (option_verbose > 2) {
			ast_verbose(VERBOSE_PREFIX_3 "MGCP mgcp_new(%s) created in state: %s\n",
				tmp->name, ast_state2str(state, 0));
		}
	} else {
...
static struct mgcp_subchannel *find_subchannel_and_lock(char *name, int msgid, struct sockaddr_in *sin)
{
	...
	while(g) {
		if ((!name || !strcasecmp(g->name, at)) && 
			...
			/* SC: no name, check if the addr matches */
			else if (!name && sin) {
...
static int transmit_notify(struct mgcp_subchannel *sub, char *event)
{
	...
	if (mgcpdebug) {
		ast_verbose(VERBOSE_PREFIX_3 "MGCP indicates event: %s on  %s@%s-%d in cxmode: %s\n", 
			event, p->name, p->parent->name, sub->id, mgcp_cxmodes[sub->cxmode]);
	}
	reqprep(&resp, p, "NTFY");
	add_header(&resp, "X", p->rqnt_ident);
	add_header(&resp, "O", event);

	/* SC: fill in new fields */
	resp.cmd = MGCP_CMD_NTFY;
	resp.trid = oseq;
...
static struct ast_channel *mgcp_request(const char *type, int format, void *data, int *cause)
{
	...
	tmpc = mgcp_new(sub, AST_STATE_DOWN);
	ast_mutex_unlock(&sub->lock);
	if (!tmpc)
...
static struct mgcp_gateway *build_gateway(char *cat, struct ast_variable *v)
{
	...
	if (gw) {
		...
		while(v) {
			if (!strcasecmp(v->name, "host")) {
				/* Make sure we become that way if we're not */
	...
	if (!ntohl(gw->addr.sin_addr.s_addr)) {
		ast_log(LOG_WARNING, "Gateway '%s' lacks call agent IP address\n", gw->name);
		if (!gw_reload) {
			ast_mutex_destroy(&gw->msgs_lock);
			free(gw);
		}
	...
	if (gw->defaddr.sin_addr.s_addr && !ntohs(gw->defaddr.sin_port)) 
		gw->defaddr.sin_port = htons(DEFAULT_MGCP_CA_PORT);
	if (gw->addr.sin_addr.s_addr && !ntohs(gw->addr.sin_port))
		gw->addr.sin_port = htons(DEFAULT_MGCP_CA_PORT);
	if (gw->addr.sin_addr.s_addr)
		if (ast_ouraddrfor(&gw->addr.sin_addr, &gw->ourip))
			memcpy(&gw->ourip, &__ourip, sizeof(gw->ourip));
	if (ast_strlen_zero(gw->domain))
	    ast_inet_ntoa(gw->domain, sizeof(gw->domain), gw->ourip);
	/* check if the domainname is numeric ip */
	if ((strchr(gw->domain, '.')) && inet_addr(gw->domain) != INADDR_NONE)
	    gw->isdomaindottedip = 1;
...
static int reload_config(void)
{
	...
	if (!ntohs(bindaddr.sin_port))
		bindaddr.sin_port = ntohs(DEFAULT_MGCP_GW_PORT);
