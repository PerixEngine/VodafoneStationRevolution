...
static char gatekeeper_id[80];
static char alternate_gatekeeper[100];
/* Find user by alias (h.323 id) is default, alternative is the incomming call's source IP address*/
static int userbyalias = 1;
static int tos = 0;
static char secret[50];
static unsigned int unique = 0;
static int tcpstart = -1;
static int tcpend = -1;

call_options_t h323_options;
...
static struct ast_rtp *oh323_get_rtp(struct ast_channel *ast);
...
static void __oh323_update_info(struct ast_channel *c, struct oh323_pvt *pvt)
{
	if (ast_codec_pref_bits(&c->nativeformats) != pvt->nativeformats) {
		...
		c->ptime = ast_get_ptime_by_format(&pvt->options.prefs, pvt->nativeformats);
...
static int oh323_digit_begin(struct ast_channel *c, char digit)
{
	...
	if (pvt->rtp && (pvt->options.dtmfmode & H323_DTMF_RFC2833)) {
		...
		if (h323debug) {
			ast_log(LOG_DEBUG, "Sending out-of-band RFC2833 digit %c on %s\n", digit, c->name);
		}
		ast_rtp_senddigit_begin(pvt->rtp, digit);
	} else if (pvt->rtp && (pvt->options.dtmfmode & H323_DTMF_INBAND)) {
		/* in-band DTMF */
		if (h323debug) {
			ast_log(LOG_DEBUG, "Sending inband digit %c on %s\n", digit, c->name);
		}
	} else {
		/* out-of-band DTMF (non rfc2833) */
		if (h323debug) {
...
{  
	...
	/* Workaround to prevent need for codec translation on transfer,
	 * by offering only the codec used in our other call. See
	 * chan_jdsp.c */
	if (pbx_builtin_getvar_helper(c, "FORCECODEC"))
		pvt->options.capability = pvt->prefcodec;
	...
	if (pvt->owner) {
		...
		if (f->frametype == AST_FRAME_VOICE) {
			if (!(f->subclass & pvt->owner->nativeformats.audio_bits)) {
				...
				pvt->owner->ptime = ast_get_ptime_by_format(&pvt->options.prefs, pvt->nativeformats);
...
static struct ast_rtp *oh323_get_rtp(struct ast_channel *chan)
{
    struct oh323_pvt *pvt = (struct oh323_pvt *) chan->tech_pvt;
...
static struct ast_channel *__oh323_new(struct oh323_pvt *pvt, int state, const char *host)
{
	...
	if (ch) {
		...
		if (pvt->options.capability)
		    ast_codec_pref_append_missing2(&ch->nativeformats, pvt->options.capability);
		else
		    ast_codec_pref_append(&ch->nativeformats, h323_options.capability);
		...
		ch->ptime = ast_get_ptime_by_format(&pvt->options.prefs, fmt);
...
static struct oh323_pvt *oh323_alloc(int callid)
{
	...
	memcpy(&pvt->options, &h323_options, sizeof(pvt->options));
	if (pvt->options.dtmfmode & H323_DTMF_RFC2833) {
		pvt->nonCodecCapability |= AST_RTP_DTMF;
	} else {
		pvt->nonCodecCapability &= ~AST_RTP_DTMF;
	}
	ast_rtp_set_inband_dtmf(pvt->rtp, pvt->options.dtmfmode &
	    H323_DTMF_INBAND);
...
static struct ast_channel *oh323_request(const char *type, const struct ast_codec_pref *formats, void *data, int *cause)
{
	...
	pvt->prefcodec = ast_best_codec(formats->audio_bits);
...
static void set_local_capabilities(unsigned call_reference, const char *token)
{
	...
	ast_mutex_unlock(&pvt->lock);
	h323_set_capabilities(token, capability, dtmfmode, prefs);
...
static int h323_registry_show(int fd, int argc, char *argv[])
{
	if (argc != 3) {
		return RESULT_SHOWUSAGE;
	}
	...
	if (gatekeeper_disable)
		ast_cli(fd, "Disabled\n");
	else if (h323_is_registered_with_gk())
		ast_cli(fd, "Registered\n");
	else
		ast_cli(fd, "Failed\n");
...
static int h323_channel_show(int fd, int argc, char *argv[])
{
	struct oh323_pvt *pvt;
	...
	for (pvt = iflist; pvt; pvt = pvt->next) {
		if (pvt->owner && !strcmp(pvt->owner->name, argv[3]))
			break;
	}
...
static char show_registry_usage[] = 
"Usage: h.323 show registry\n"
"       Show gatekeeper registration status\n";
...
"Usage: h.323 show channel\n"
"       Provides detailed status on a given H.323 channel\n";
...
static struct ast_cli_entry  cli_show_registry =
	{ { "h.323", "show", "registry", NULL }, h323_registry_show, "Show gatekeeper registration status", show_registry_usage };
...
static struct ast_cli_entry  cli_show_channel =
	{ { "h.323", "show", "channel", NULL }, h323_channel_show, "Show channel", show_channel_usage };
...
static int update_common_options(struct ast_variable *v, struct call_options *options)
{
	...
	if (!strcasecmp(v->name, "allow")) {
		...
		if (!strcasecmp(v->value, "inband")) {
			...
			options->dtmfmode = H323_DTMF_Q931_KEYPAD;
		} else if (!strcasecmp(v->value, "h245alphanumeric")) {
			options->dtmfmode = H323_DTMF_H245_ALPHANUMERIC;
		} else if (!strcasecmp(v->value, "h245signal")) {
			options->dtmfmode = H323_DTMF_H245_SIGNAL;
		} else {
...
static struct oh323_user *build_user(char *name, struct ast_variable *v)
{
	...
	if (user) {
		...
		memcpy(&user->options, &h323_options, sizeof(user->options));
		/* Set default context */
...
static struct oh323_peer *build_peer(char *name, struct ast_variable *v)
{
	...
	if (peer) {
		...
		peer->ha = NULL;
		peer->addr.sin_family = AF_INET;
		memcpy(&peer->options, &h323_options, sizeof(peer->options));
...
{	
	...
	memset(&h323_options, 0, sizeof(h323_options));
	h323_options.dtmfcodec = 101;
	h323_options.dtmfmode = H323_DTMF_RFC2833;
	h323_options.capability = ast_getformatbyname("all"); /* All capabilities */
	h323_options.bridge = 1;		/* Do native bridging by default */
	...
	while(v) {
		...
		if (!strcasecmp(v->name, "port")) {
			...
			strncpy(gatekeeper_id, v->value, sizeof(gatekeeper_id) - 1);
		} else if (!strcasecmp(v->name, "timetolive")) {
			h323_set_gk_time_to_live(atoi(v->value));
		} else if (!strcasecmp(v->name, "alternategatekeeper")) {
			strncpy(alternate_gatekeeper, v->value, sizeof(alternate_gatekeeper) - 1);
		} else if (!strcasecmp(v->name, "secret")) {
		...
		} else if (!update_common_options(v, &h323_options)) {
			/* dummy */
		} else if (!strcasecmp(v->name, "tcpstart")) {
		    tcpstart = atoi(v->value);
	...
	if (tcpstart != -1 && tcpend != -1)
	    h323_set_tcp_ports(tcpstart, tcpend);
...
int load_module()
{
	...
	if (res) {
		...
		/* Possibly register with a GK */
		if (!gatekeeper_disable) {
			if (h323_set_gk(gatekeeper_discover, gatekeeper, gatekeeper_id,
				secret, alternate_gatekeeper)) {
				ast_log(LOG_ERROR, "Gatekeeper registration failed.\n");
				return 0;
			}
		}
