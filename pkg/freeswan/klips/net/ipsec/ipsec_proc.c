/*
 * @(#) /proc file system interface code.
 *
 * Copyright (C) 1996, 1997  John Ioannidis.
 * Copyright (C) 1998, 1999, 2000, 2001  Richard Guy Briggs <rgb@freeswan.org>
 *                                 2001  Michael Richardson <mcr@freeswan.org>
 * 
 * This program is free software; you can redistribute it and/or modify it
 * under the terms of the GNU General Public License as published by the
 * Free Software Foundation; either version 2 of the License, or (at your
 * option) any later version.  See <http://www.fsf.org/copyleft/gpl.txt>.
 * 
 * This program is distributed in the hope that it will be useful, but
 * WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY
 * or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License
 * for more details.
 *
 * Split out from ipsec_init.c version 1.70.
 */

#include <linux/version.h>
#define __NO_VERSION__
#include <linux/module.h>
#include <linux/kernel.h> /* printk() */

#include "ipsec_param.h"

#ifdef MALLOC_SLAB
# include <linux/slab.h> /* kmalloc() */
#else /* MALLOC_SLAB */
# include <linux/malloc.h> /* kmalloc() */
#endif /* MALLOC_SLAB */
#include <linux/errno.h>  /* error codes */
#include <linux/types.h>  /* size_t */
#include <linux/interrupt.h> /* mark_bh */

#include <linux/netdevice.h>   /* struct net_device, and other headers */
#include <linux/etherdevice.h> /* eth_type_trans */
#include <linux/ip.h>          /* struct iphdr */
#include <linux/in.h>          /* struct sockaddr_in */
#include <linux/skbuff.h>
#include <freeswan.h>
#ifdef SPINLOCK
#ifdef SPINLOCK_23
#include <linux/spinlock.h> /* *lock* */
#else /* SPINLOCK_23 */
#include <asm/spinlock.h> /* *lock* */
#endif /* SPINLOCK_23 */
#endif /* SPINLOCK */
#ifdef NET_21
#include <asm/uaccess.h>
#include <linux/in6.h>
#endif /* NET_21 */
#include <asm/checksum.h>
#include <net/ip.h>
#ifdef CONFIG_PROC_FS
#include <linux/proc_fs.h>
#endif /* CONFIG_PROC_FS */
#ifdef NETLINK_SOCK
#include <linux/netlink.h>
#else
#include <net/netlink.h>
#endif

#include "radij.h"

#include "ipsec_life.h"
#include "ipsec_stats.h"
#include "ipsec_sa.h"

#include "ipsec_encap.h"
#include "ipsec_radij.h"
#include "ipsec_netlink.h"
#include "ipsec_xform.h"
#include "ipsec_tunnel.h"

#include "ipsec_rcv.h"
#include "ipsec_ah.h"
#include "ipsec_esp.h"

#ifdef CONFIG_IPSEC_IPCOMP
#include "ipcomp.h"
#endif /* CONFIG_IPSEC_IPCOMP */

#include "ipsec_proto.h"
#include "ipsec_reject_debug.h"
#include "ipsec_log.h"

#include <pfkeyv2.h>
#include <pfkey.h>

#ifdef CONFIG_PROC_FS

#define BUF_PRINT(args...) \
do { \
	n = snprintf(buffer + len, count - len, ##args); \
	if (n + len >= count) \
	    goto Exit; \
	len += n; \
} while (0)

#if 0
static struct proc_dir_entry *proc_net_ipsec_dir;
#endif

IPSEC_PROCFS_DEBUG_NO_STATIC
int
ipsec_eroute_read_proc(char *buffer,
		       char **start,
		       off_t offset,
		       int count,
		       int *peof,
		       void *dat)
{
	struct wsbuf w = {buffer, count, offset, 0, 0, 0, 0};
	int error = 0;

#ifdef CONFIG_IPSEC_DEBUG
	if (debug_radij & DB_RJ_DUMPTREES)
	  rj_dumptrees();			/* XXXXXXXXX */
#endif /* CONFIG_IPSEC_DEBUG */

	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_eroute_get_info: "
		    "buffer=0p%p, *start=0p%p, offset=%d, count=%d\n",
		    buffer,
		    *start,
		    (int)offset,
		    count);

	*peof = 0;

	spin_lock_bh(&eroute_lock);

	error = rj_walktree(rnh, ipsec_rj_walker_procprint, &w);
/*	rj_walktree(mask_rjhead, ipsec_rj_walker_procprint, &w); */

	spin_unlock_bh(&eroute_lock);

	if (!error)
	    *peof = 1;

	*start = buffer + (offset - w.begin);	/* Start of wanted data */
	w.len -= (offset - w.begin);			/* Start slop */
	if (w.len > count)
		w.len = count;
	if (w.len < 0)
		w.len = 0;
	return w.len;
}

IPSEC_PROCFS_DEBUG_NO_STATIC
int
ipsec_spi_read_proc(char *buffer,
		   char **start,
		   off_t offset,
		   int count,
		   int *peof,
		   void *dat)
{
	int len = 0;
	off_t pos = 0, begin = 0;
	int i;
	struct ipsec_sa *sa_p;
	char sa[SATOA_BUF];
	char buf_s[SUBNETTOA_BUF];
	char buf_d[SUBNETTOA_BUF];
	size_t sa_len;

#define SNPRINTF_SAFE(fmt...) if (len >= count) goto done_one_spi; \
	len += snprintf(buffer + len, count - len, fmt);

	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_spi_get_info: "
		    "buffer=0p%p, *start=0p%p, offset=%d, count=%d\n",
		    buffer,
		    *start,
		    (int)offset,
		    count);
	
	*peof = 0;

	spin_lock_bh(&tdb_lock);


	
	for (i = 0; i < SADB_HASHMOD; i++) {
		for (sa_p = ipsec_sadb_hash[i];
		     sa_p;
		     sa_p = sa_p->ips_hnext) {
			sa_len = satoa(sa_p->ips_said, 0, sa, SATOA_BUF);
			SNPRINTF_SAFE("%s ",
				       sa_len ? sa : " (error)");

			SNPRINTF_SAFE("%s%s%s",
				       IPS_XFORM_NAME(sa_p));

			SNPRINTF_SAFE(": dir=%s",
				       (sa_p->ips_flags & EMT_INBOUND) ?
				       "in " : "out");

			if(sa_p->ips_addr_s) {
				addrtoa(((struct sockaddr_in*)(sa_p->ips_addr_s))->sin_addr,
					0, buf_s, sizeof(buf_s));
				SNPRINTF_SAFE(" src=%s",
					       buf_s);
			}

			if((sa_p->ips_said.proto == IPPROTO_IPIP)
			   && (sa_p->ips_flags & SADB_X_SAFLAGS_INFLOW)) {
				subnettoa(sa_p->ips_flow_s.u.v4.sin_addr,
					  sa_p->ips_mask_s.u.v4.sin_addr,
					  0,
					  buf_s,
					  sizeof(buf_s));

				subnettoa(sa_p->ips_flow_d.u.v4.sin_addr,
					  sa_p->ips_mask_d.u.v4.sin_addr,
					  0,
					  buf_d,
					  sizeof(buf_d));

				SNPRINTF_SAFE(" policy=%s->%s",
					       buf_s, buf_d);
			}
			
			if(sa_p->ips_iv_bits) {
				int j;
				SNPRINTF_SAFE(" iv_bits=%dbits iv=0x",
					       sa_p->ips_iv_bits);

				for(j = 0; j < sa_p->ips_iv_bits / 8; j++) {
					SNPRINTF_SAFE("%02x",
						       (__u32)((__u8*)(sa_p->ips_iv))[j]);
				}
			}

			if(sa_p->ips_encalg || sa_p->ips_authalg) {
				if(sa_p->ips_replaywin) {
					SNPRINTF_SAFE(" ooowin=%d",
						       sa_p->ips_replaywin);
				}
				if(sa_p->ips_errs.ips_replaywin_errs) {
					SNPRINTF_SAFE(" ooo_errs=%d",
						       sa_p->ips_errs.ips_replaywin_errs);
				}
				if(sa_p->ips_replaywin_lastseq) {
                                       SNPRINTF_SAFE(" seq=%d",
						      sa_p->ips_replaywin_lastseq);
				}
				if(sa_p->ips_replaywin_bitmap) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,3,0)
					SNPRINTF_SAFE(" bit=0x%Lx",
						       sa_p->ips_replaywin_bitmap);
#else
					SNPRINTF_SAFE(" bit=0x%x%08x",
						       (__u32)(sa_p->ips_replaywin_bitmap >> 32),
						       (__u32)sa_p->ips_replaywin_bitmap);
#endif
				}
				if(sa_p->ips_replaywin_maxdiff) {
					SNPRINTF_SAFE(" max_seq_diff=%d",
						       sa_p->ips_replaywin_maxdiff);
				}
			}
			if(sa_p->ips_flags & ~EMT_INBOUND) {
				SNPRINTF_SAFE(" flags=0x%x",
					       sa_p->ips_flags & ~EMT_INBOUND);
				SNPRINTF_SAFE("<");
				/* flag printing goes here */
				SNPRINTF_SAFE(">");
			}
			if(sa_p->ips_auth_bits) {
				SNPRINTF_SAFE(" alen=%d",
					       sa_p->ips_auth_bits);
			}
			if(sa_p->ips_key_bits_a) {
				SNPRINTF_SAFE(" aklen=%d",
					       sa_p->ips_key_bits_a);
			}
			if(sa_p->ips_errs.ips_auth_errs) {
				SNPRINTF_SAFE(" auth_errs=%d",
					       sa_p->ips_errs.ips_auth_errs);
			}
			if(sa_p->ips_key_bits_e) {
				SNPRINTF_SAFE(" eklen=%d",
					       sa_p->ips_key_bits_e);
			}
			if(sa_p->ips_errs.ips_encsize_errs) {
				SNPRINTF_SAFE(" encr_size_errs=%d",
					       sa_p->ips_errs.ips_encsize_errs);
			}
			if(sa_p->ips_errs.ips_encpad_errs) {
				SNPRINTF_SAFE(" encr_pad_errs=%d",
					       sa_p->ips_errs.ips_encpad_errs);
			}
			
			SNPRINTF_SAFE(" life(c,s,h)=");

			len += ipsec_lifetime_format(buffer + len,
						     count - len,
						     "alloc", 
						     ipsec_life_countbased,
						     &sa_p->ips_life.ipl_allocations);

			len += ipsec_lifetime_format(buffer + len,
						     count - len,
						     "bytes",
						     ipsec_life_countbased,
						     &sa_p->ips_life.ipl_bytes);

			len += ipsec_lifetime_format(buffer + len,
						     count - len,
						     "addtime",
						     ipsec_life_timebased,
						     &sa_p->ips_life.ipl_addtime);

			len += ipsec_lifetime_format(buffer + len,
						     count - len,
						     "usetime",
						     ipsec_life_timebased,
						     &sa_p->ips_life.ipl_usetime);
			
			len += ipsec_lifetime_format(buffer + len,
						     count - len,
						     "packets",
						     ipsec_life_countbased,
						     &sa_p->ips_life.ipl_packets);
			
			if(sa_p->ips_life.ipl_usetime.ipl_last) { /* XXX-MCR should be last? */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,3,0)
				SNPRINTF_SAFE(" idle=%Ld",
					       jiffies / HZ - sa_p->ips_life.ipl_usetime.ipl_last);
#else
				SNPRINTF_SAFE(" idle=%lu",
					       jiffies / HZ - (unsigned long)sa_p->ips_life.ipl_usetime.ipl_last);
#endif
			}

#ifdef CONFIG_IPSEC_IPCOMP
			if(sa_p->ips_said.proto == IPPROTO_COMP &&
			   (sa_p->ips_comp_ratio_dbytes ||
			    sa_p->ips_comp_ratio_cbytes)) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,3,0)
				SNPRINTF_SAFE(" ratio=%Ld:%Ld",
					       sa_p->ips_comp_ratio_dbytes,
					       sa_p->ips_comp_ratio_cbytes);
#else
				SNPRINTF_SAFE(" ratio=%lu:%lu",
					       (unsigned long)sa_p->ips_comp_ratio_dbytes,
					       (unsigned long)sa_p->ips_comp_ratio_cbytes);
#endif
			}
#endif /* CONFIG_IPSEC_IPCOMP */

#ifdef CONFIG_IPSEC_NAT_TRAVERSAL
			{
				char *natttype_name;

				switch(sa_p->ips_natt_type)
				{
				case 0:
					natttype_name="none";
					break;
				case ESPINUDP_WITH_NON_IKE:
					natttype_name="nonike";
					break;
				case ESPINUDP_WITH_NON_ESP:
					natttype_name="nonesp";
					break;
				default:
					natttype_name = "unknown";
					break;
				}

				SNPRINTF_SAFE(" natencap=%s",
					       natttype_name);
				SNPRINTF_SAFE(" natsport=%d",
					       sa_p->ips_natt_sport);
				SNPRINTF_SAFE(" natdport=%d",
					       sa_p->ips_natt_dport);
			}
#else
			SNPRINTF_SAFE(" natencap=na");
#endif /* CONFIG_IPSEC_NAT_TRAVERSAL */

			SNPRINTF_SAFE(" orig_bytes=%lu",
			                sa_p->ips_life.orig_bytes);
			SNPRINTF_SAFE(" orig_packets=%lu",
			                sa_p->ips_life.orig_packets);
			SNPRINTF_SAFE(" orig_errors=%lu",
			                sa_p->ips_life.orig_errors);
			SNPRINTF_SAFE(" orig_dropped=%lu",
			                sa_p->ips_life.orig_dropped);

			SNPRINTF_SAFE("\n");

done_one_spi:
			pos = begin + len;
			if(pos < offset) {
				len = 0;
				begin = pos;
			}
			if (pos >= offset + count) {
				goto done_spi_i;
			}
		}
	}

 done_spi_i:	
	spin_unlock_bh(&tdb_lock);

	*start = buffer + (offset - begin);	/* Start of wanted data */
	len -= (offset - begin);			/* Start slop */
	if (len > count)
		len = count;
	else
		*peof = 1;

	if (len < 0)
		len = 0;
	return len;
}

IPSEC_PROCFS_DEBUG_NO_STATIC
int
ipsec_spigrp_read_proc(char *buffer,
		      char **start,
		      off_t offset,
		      int count,
		      int *peof,
		      void *dat)
{
	int len = 0;
	off_t pos = 0, begin = 0;
	int i;
	struct ipsec_sa *sa_p, *sa_p2;
	char sa[SATOA_BUF];
	size_t sa_len;
	int n = 0;

	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_spigrp_get_info: "
		    "buffer=0p%p, *start=0p%p, offset=%d, count=%d\n",
		    buffer,
		    *start,
		    (int)offset,
		    count);

	*peof = 0;

	spin_lock_bh(&tdb_lock);
	
	for (i = 0; i < SADB_HASHMOD; i++) {
		for (sa_p = ipsec_sadb_hash[i];
		     sa_p;
		     sa_p = sa_p->ips_hnext)
		{
			if(!sa_p->ips_inext)
			{
				sa_p2 = sa_p;
				while(sa_p2) {
					sa_len = satoa(sa_p2->ips_said,
						       0, sa, SATOA_BUF);
					
					BUF_PRINT("%s ",
					    sa_len ? sa : " (error)");
					sa_p2 = sa_p2->ips_onext;
				}
				BUF_PRINT("\n");
				pos = begin + len;
				if(pos < offset) {
					len = 0;
					begin = pos;
				}
				if (pos > offset + count) {
					goto done_spigrp_i;
				}
			}
		}
	}

	*peof = 1;

Exit:
 done_spigrp_i:	
	spin_unlock_bh(&tdb_lock);

	*start = buffer + (offset - begin);	/* Start of wanted data */
	len -= (offset - begin);			/* Start slop */
	if (len < 0)
	    len = 0;
	return len;
}

IPSEC_PROCFS_DEBUG_NO_STATIC
int
ipsec_tncfg_read_proc(char *buffer,
		      char **start,
		      off_t offset,
		      int count,
		      int *peof,
		      void *dat)
{
	int len = 0;
	off_t pos = 0, begin = 0;
	struct net_device *dev, *privdev;
	struct ipsecpriv *priv;
	ipsec_dev_list *cur;
	int n = 0;

	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_tncfg_get_info: "
		    "buffer=0p%p, *start=0p%p, offset=%d, count=%d\n",
		    buffer,
		    *start,
		    (int)offset,
		    count);

	*peof = 0;

	for(cur=ipsec_dev_head; cur; cur=cur->next) {
		dev = ipsec_dev_get(cur->ipsec_dev->name);
		if(dev) {
			priv = (struct ipsecpriv *)netdev_priv(dev);
			BUF_PRINT("%s", dev->name);
			if(priv) {
				privdev = (struct net_device *)(priv->dev);
				BUF_PRINT(" -> %s", privdev ? privdev->name :
				    "NULL");
				BUF_PRINT(" mtu=%d(%d) -> %d",
					       dev->mtu,
					       priv->mtu,
					       privdev ? privdev->mtu : 0);
			} else {
				KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
					    "klips_debug:ipsec_tncfg_get_info: device '%s' has no private data space!\n",
					    dev->name);
			}
			BUF_PRINT("\n");

			pos = begin + len;
			if(pos < offset) {
				len = 0;
				begin = pos;
			}
			else if (pos > offset + count)	{
				break;
			}
		}
	}

	*peof = 1;

Exit:
	*start = buffer + (offset - begin);	/* Start of wanted data */
	len -= (offset - begin);			/* Start slop */
	if (len < 0)
		len = 0;
	return len;
}

IPSEC_PROCFS_DEBUG_NO_STATIC
int
ipsec_version_read_proc(char *buffer,
		        char **start,
		        off_t offset,
		        int count,
		        int *peof,
		        void *dat)
{
	int len = 0;
	off_t begin = 0;
	int n = 0;

	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_version_get_info: "
		    "buffer=0p%p, *start=0p%p, offset=%d, count=%d\n",
		    buffer,
		    *start,
		    (int)offset,
		    count);

	*peof = 0;

	BUF_PRINT("FreeS/WAN version: %s\n", ipsec_version_code());
#if 0
	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_version_get_info: "
		    "ipsec_init version: %s\n",
		    ipsec_init_c_version);
	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_version_get_info: "
		    "ipsec_tunnel version: %s\n",
		    ipsec_tunnel_c_version);
	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_version_get_info: "
		    "ipsec_netlink version: %s\n",
		    ipsec_netlink_c_version);
	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_version_get_info: "
		    "radij_c_version: %s\n",
		    radij_c_version);
#endif
	*peof = 1;

Exit:
	*start = buffer + (offset - begin);	/* Start of wanted data */
	len -= (offset - begin);			/* Start slop */
	if (len < 0)
		len = 0;
	return len;
}

IPSEC_PROCFS_DEBUG_NO_STATIC
int
ipsec_natt_read_proc(char *buffer,
		     char **start,
		     off_t offset,
		     int count,
		     int *peof,
		     void *dat)
{
	int len = 0;
	off_t begin = 0;
	int n = 0;

	*peof = 0;

	BUF_PRINT("%d\n",
#ifdef CONFIG_IPSEC_NAT_TRAVERSAL
			      1
#else
			      0
#endif
		);

	*peof = 1;

Exit:
	*start = buffer + (offset - begin);	/* Start of wanted data */
	len -= (offset - begin);			/* Start slop */
	if (len < 0)
		len = 0;
	return len;
}

#ifdef CONFIG_IPSEC_DEBUG
IPSEC_PROCFS_DEBUG_NO_STATIC
int
ipsec_klipsdebug_read_proc(char *buffer,
			   char **start,
			   off_t offset,
			   int count,
			   int *peof,
			   void *dat)
{
	int len = 0;
	off_t begin = 0;
	int n = 0;

	KLIPS_PRINT(debug_tunnel & DB_TN_PROCFS,
		    "klips_debug:ipsec_klipsdebug_get_info: "
		    "buffer=0p%p, *start=0p%p, offset=%d, count=%d\n",
		    buffer,
		    *start,
		    (int)offset,
		    count);

	*peof = 0;

	BUF_PRINT("debug_tunnel=%08x.\n", debug_tunnel);
	BUF_PRINT("debug_netlink=%08x.\n", debug_netlink);
	BUF_PRINT("debug_xform=%08x.\n", debug_xform);
	BUF_PRINT("debug_eroute=%08x.\n", debug_eroute);
	BUF_PRINT("debug_spi=%08x.\n", debug_spi);
	BUF_PRINT("debug_radij=%08x.\n", debug_radij);
	BUF_PRINT("debug_esp=%08x.\n", debug_esp);
	BUF_PRINT("debug_ah=%08x.\n", debug_ah);
	BUF_PRINT("debug_rcv=%08x.\n", debug_rcv);
	BUF_PRINT("debug_pfkey=%08x.\n", debug_pfkey);
#ifdef CONFIG_IPSEC_IPCOMP
	BUF_PRINT("debug_ipcomp=%08x.\n", sysctl_ipsec_debug_ipcomp);
#endif
	BUF_PRINT("debug_verbose=%08x.\n", sysctl_ipsec_debug_verbose);
	BUF_PRINT("debug_reject=%08x.\n", debug_reject);
	BUF_PRINT("debug_log_all=%08x.\n", debug_log_all);

	*peof = 1;

Exit:
	*start = buffer + (offset - begin);	/* Start of wanted data */
	len -= (offset - begin);			/* Start slop */
	if (len < 0)
	    len = 0;
	return len;
}
#endif /* CONFIG_IPSEC_DEBUG */

#ifndef PROC_FS_2325
struct proc_dir_entry ipsec_eroute =
{
	0,
	12, "ipsec_eroute",
	S_IFREG | S_IRUGO, 1, 0, 0, 0,
	&proc_net_inode_operations,
	ipsec_eroute_get_info,
	NULL, NULL, NULL, NULL, NULL
};

struct proc_dir_entry ipsec_spi =
{
	0,
	9, "ipsec_spi",
	S_IFREG | S_IRUGO, 1, 0, 0, 0,
	&proc_net_inode_operations,
	ipsec_spi_get_info,
	NULL, NULL, NULL, NULL, NULL
};

struct proc_dir_entry ipsec_spigrp =
{
	0,
	12, "ipsec_spigrp",
	S_IFREG | S_IRUGO, 1, 0, 0, 0,
	&proc_net_inode_operations,
	ipsec_spigrp_get_info,
	NULL, NULL, NULL, NULL, NULL
};

struct proc_dir_entry ipsec_tncfg =
{
	0,
	11, "ipsec_tncfg",
	S_IFREG | S_IRUGO, 1, 0, 0, 0,
	&proc_net_inode_operations,
	ipsec_tncfg_get_info,
	NULL, NULL, NULL, NULL, NULL
};

struct proc_dir_entry ipsec_version =
{
	0,
	13, "ipsec_version",
	S_IFREG | S_IRUGO, 1, 0, 0, 0,
	&proc_net_inode_operations,
	ipsec_version_get_info,
	NULL, NULL, NULL, NULL, NULL
};

struct proc_dir_entry ipsec_natt =
{
	0,
	13, "ipsec_natt",
	S_IFREG | S_IRUGO, 1, 0, 0, 0,
	&proc_net_inode_operations,
	ipsec_natt_get_info,
	NULL, NULL, NULL, NULL, NULL
};

#ifdef CONFIG_IPSEC_DEBUG
struct proc_dir_entry ipsec_klipsdebug =
{
	0,
	16, "ipsec_klipsdebug",
	S_IFREG | S_IRUGO, 1, 0, 0, 0,
	&proc_net_inode_operations,
	ipsec_klipsdebug_get_info,
	NULL, NULL, NULL, NULL, NULL
};
#endif /* CONFIG_IPSEC_DEBUG */
#endif /* !PROC_FS_2325 */
#endif /* CONFIG_PROC_FS */

int
ipsec_proc_init()
{
	int error = 0;

	/* compile a dummy function if no /proc/-fs */

	/* XXX-mcr probably should just complain because pluto won't
	 * run without /proc!
	 */

#ifdef CONFIG_PROC_FS 
#  ifndef PROC_FS_2325
#    ifdef PROC_FS_21
	error |= proc_register(proc_net, &ipsec_eroute);
	error |= proc_register(proc_net, &ipsec_spi);
	error |= proc_register(proc_net, &ipsec_spigrp);
	error |= proc_register(proc_net, &ipsec_tncfg);
	error |= proc_register(proc_net, &ipsec_version);
#      ifdef CONFIG_IPSEC_DEBUG
	error |= proc_register(proc_net, &ipsec_klipsdebug);
#      endif /* CONFIG_IPSEC_DEBUG */
#    else /* PROC_FS_21 */
	error |= proc_register_dynamic(&proc_net, &ipsec_eroute);
	error |= proc_register_dynamic(&proc_net, &ipsec_spi);
	error |= proc_register_dynamic(&proc_net, &ipsec_spigrp);
	error |= proc_register_dynamic(&proc_net, &ipsec_tncfg);
	error |= proc_register_dynamic(&proc_net, &ipsec_version);
#      ifdef CONFIG_IPSEC_DEBUG
	error |= proc_register_dynamic(&proc_net, &ipsec_klipsdebug);
#      endif /* CONFIG_IPSEC_DEBUG */
#    endif /* PROC_FS_21 */
#  else /* !PROC_FS_2325 */
	/* create /proc/net/ipsec */
#if 0
	proc_net_ipsec_dir = proc_mkdir("ipsec", proc_net);
	if(proc_net_ipsec_dir == NULL) {
		error |= 1;
	} else{
		error |= proc_register();
	}		
#endif

	create_proc_read_entry("ipsec_eroute", S_IRUGO, PROC_NET,
	    ipsec_eroute_read_proc, NULL);
	create_proc_read_entry("ipsec_spi", S_IRUGO, PROC_NET,
	    ipsec_spi_read_proc, NULL);
	create_proc_read_entry("ipsec_spigrp", S_IRUGO, PROC_NET,
	    ipsec_spigrp_read_proc, NULL);
	create_proc_read_entry("ipsec_tncfg", S_IRUGO, PROC_NET,
	    ipsec_tncfg_read_proc, NULL);
	create_proc_read_entry("ipsec_version", S_IRUGO, PROC_NET,
	    ipsec_version_read_proc, NULL);
	create_proc_read_entry("ipsec_natt", S_IRUGO, PROC_NET,
	    ipsec_natt_read_proc, NULL);
#    ifdef CONFIG_IPSEC_DEBUG
	create_proc_read_entry("ipsec_klipsdebug", S_IRUGO, PROC_NET,
	    ipsec_klipsdebug_read_proc, NULL);
#    endif /* CONFIG_IPSEC_DEBUG */
#  endif /* !PROC_FS_2325 */
#endif /* CONFIG_PROC_FS */

	return error;
}

void
ipsec_proc_cleanup()
{
#ifdef CONFIG_PROC_FS
#  ifndef PROC_FS_2325
#    ifdef CONFIG_IPSEC_DEBUG
	if (proc_net_unregister(ipsec_klipsdebug.low_ino) != 0)
		ipsec_log("klips_debug:ipsec_cleanup: "
		       "cannot unregister /proc/net/ipsec_klipsdebug\n");
#    endif /* CONFIG_IPSEC_DEBUG */
	if (proc_net_unregister(ipsec_version.low_ino) != 0)
		ipsec_log("klips_debug:ipsec_cleanup: "
		       "cannot unregister /proc/net/ipsec_version\n");
	if (proc_net_unregister(ipsec_eroute.low_ino) != 0)
		ipsec_log("klips_debug:ipsec_cleanup: "
		       "cannot unregister /proc/net/ipsec_eroute\n");
	if (proc_net_unregister(ipsec_spi.low_ino) != 0)
		ipsec_log("klips_debug:ipsec_cleanup: "
		       "cannot unregister /proc/net/ipsec_spi\n");
	if (proc_net_unregister(ipsec_spigrp.low_ino) != 0)
		ipsec_log("klips_debug:ipsec_cleanup: "
		       "cannot unregister /proc/net/ipsec_spigrp\n");
	if (proc_net_unregister(ipsec_tncfg.low_ino) != 0)
		ipsec_log("klips_debug:ipsec_cleanup: "
		       "cannot unregister /proc/net/ipsec_tncfg\n");
#  else /* !PROC_FS_2325 */
#    ifdef CONFIG_IPSEC_DEBUG
	ipsec_proc_net_remove ("ipsec_klipsdebug");
#    endif /* CONFIG_IPSEC_DEBUG */
	ipsec_proc_net_remove ("ipsec_eroute");
	ipsec_proc_net_remove ("ipsec_spi");
	ipsec_proc_net_remove ("ipsec_spigrp");
	ipsec_proc_net_remove ("ipsec_tncfg");
	ipsec_proc_net_remove ("ipsec_version");
#  endif /* !PROC_FS_2325 */
#endif          /* CONFIG_PROC_FS */
}

/*
 * $Log: ipsec_proc.c,v $
 * Revision 1.15.4.2  2015/07/13 13:09:19  itamarh
 * B191157 Change freeswan proc API so it's compatible with later kernels as well
 *
 * Revision 1.15.2.2  2015/07/13 13:04:29  itamarh
 * B191157 Change freeswan proc API so it's compatible with later kernels as well
 *
 * Revision 1.15.4.1  2015/07/01 13:35:03  itamarh
 * AUTO MERGE: branch-5_4_8_1 <- branch-5_4_8_1_236_1
 * B191157 Replace dev->priv usage in freeswan with netdev_priv
 * NOTIFY: automerge
 *
 * Revision 1.15.2.1  2015/07/01 13:34:08  itamarh
 * B191157 Replace dev->priv usage in freeswan with netdev_priv
 *
 * Revision 1.15  2010/04/08 20:13:26  dmitri
 * B92240: Merge new kernel related changes from 5.3.6.1 to dev (merge 5.3 -> 1)
 * NOTIFY: cancel
 *
 * Revision 1.14.16.1  2010/04/06 19:44:25  dmitri
 * B92240: Merge new kernel related changes from 5.3.6.1 to dev
 * NOTIFY: cancel
 *
 * Revision 1.14.14.1  2010/03/29 10:32:39  dmitri
 * B92240: Merge linux kernel 2.6.21.5 upgrade related changes to 5.3.6.1 - part 4
 * NOTIFY: cancel
 *
 * Revision 1.14.4.1  2009/09/29 09:00:51  shayh
 * B83196 get rid of linux/config.h inclusion. Instead, add -include for
 * include/linux/autoconf.h in linux's CFLAGS options. Based on R47009.
 *
 * Revision 1.14  2007/09/24 17:06:16  itay
 * B50901 merge Mindspeed Comcerto-100 code to product
 *
 * Revision 1.13.2.1  2007/09/24 16:33:09  itay
 * B50901 merge Mindspeed Comcerto-100 code to product
 *
 * Revision 1.13  2007/07/12 11:11:07  olegy
 * B47532 CVS Log fix
 *
 * Revision 1.10  2007/06/11 11:46:11  guyn
 * B43992 B45894 rgload failed on malindi2_4_7 at dev. Revert the work done on
 * B43992 (Intel Hamoa reference platform porting), in order for MALINDI2 to work.
 *
 * Revision 1.8  2006/02/23 17:36:43  sergey
 * AUTO MERGE: 1 <- branch-4_2
 * B6059: implement (OpenSWAN based) IPSec NAT-Traversal.
 * NOTIFY: automerge
 *
 * Revision 1.7.2.1  2006/02/23 17:32:13  sergey
 * B6059: implement (OpenSWAN based) IPSec NAT-Traversal.
 *
 * Revision 1.7  2005/11/14 10:54:29  sergey
 * AUTO MERGE: 1 <- branch-4_1
 * B26997: enable IPSec with linux 2.6.
 * NOTIFY: automerge
 *
 * Revision 1.6.162.1  2005/11/14 10:47:20  sergey
 * B26997: enable IPSec with linux 2.6.
 *
 * Revision 1.6  2004/01/18 16:36:47  sergey
 * FIX: B7835. Added rate limit to IPSec log.
 *
 * Revision 1.5  2003/09/21 20:23:15  igork
 * merge branch-dev-2421 into dev
 *
 * Revision 1.4  2003/09/21 15:13:30  sergey
 * FIX: B5670. Added log for rejected IPSec packets (ICSA).
 *
 * Revision 1.2.16.1  2003/09/16 13:34:20  ron
 * merge from branch-3_1 to merge-dev-branch-dev-2421 into branch-dev-2421
 *
 * Revision 1.3  2003/09/11 11:37:56  yoavp
 * AUTO MERGE: 1 <- branch-3_2
 * R7234: add NONE to range-types, remove RCSIDs from freeswan and some cosmetics
 * NOTIFY: automerge
 *
 * Revision 1.2.18.1  2003/09/11 11:34:41  yoavp
 * R7234: add NONE to range-types, remove RCSIDs from freeswan and some cosmetics
 *
 * Revision 1.2  2003/05/26 19:01:27  eyalm
 * Merge branch-2_5-start to tag-5_2_2 into dev
 *
 * Revision 1.1  2003/02/19 11:46:31  sergey
 * Initial revision
 *
 * Revision 1.8  2002/01/29 17:17:55  mcr
 * 	moved include of ipsec_param.h to after include of linux/kernel.h
 * 	otherwise, it seems that some option that is set in ipsec_param.h
 * 	screws up something subtle in the include path to kernel.h, and
 * 	it complains on the snprintf() prototype.
 *
 * Revision 1.7  2002/01/29 04:00:52  mcr
 * 	more excise of kversions.h header.
 *
 * Revision 1.6  2002/01/29 02:13:17  mcr
 * 	introduction of ipsec_kversion.h means that include of
 * 	ipsec_param.h must preceed any decisions about what files to
 * 	include to deal with differences in kernel source.
 *
 * Revision 1.5  2002/01/12 02:54:30  mcr
 * 	beginnings of /proc/net/ipsec dir.
 *
 * Revision 1.4  2001/12/11 02:21:05  rgb
 * Don't include module version here, fixing 2.2 compile bug.
 *
 * Revision 1.3  2001/12/05 07:19:44  rgb
 * Fixed extraneous #include "version.c" bug causing modular KLIPS failure.
 *
 * Revision 1.2  2001/11/26 09:16:14  rgb
 * Merge MCR's ipsec_sa, eroute, proc and struct lifetime changes.
 *
 * Revision 1.74  2001/11/22 05:44:11  henry
 * new version stuff
 *
 * Revision 1.1.2.1  2001/09/25 02:19:40  mcr
 * 	/proc manipulation code moved to new ipsec_proc.c
 *
 *
 * Local variables:
 * c-file-style: "linux"
 * End:
 *
 */
