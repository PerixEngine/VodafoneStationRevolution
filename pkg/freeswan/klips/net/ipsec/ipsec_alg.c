 ...
 * Version: 0.8.1
 *
 * $Id: ipsec_alg.c,v 1.10.4.1 2015/07/13 14:13:55 itamarh Exp $
 *
...
/*
 * 	joint encrypt+auth key context creation function
 * 	called from pfkey_v2_parser.c:pfkey_ips_init() 
 */
int ipsec_alg_joint_key_create(struct ipsec_sa *sa_p) {
	int ret=-EINVAL;
	struct ipsec_alg_joint *ixt_j=sa_p->ips_alg_joint;

	sa_p->ips_j_ctx_size = ixt_j->ixt_j_ctx_size;
	
	if((sa_p->ips_j_ctx = 
		(caddr_t) kmalloc(sa_p->ips_j_ctx_size, GFP_ATOMIC)) == NULL) {
		ret=-ENOMEM;
		goto ixt_out;
	}
	/* ips_j_ctx is used by registered JOINT client. At this point it can
	 * be used to pass to client any data (only Comcerto need this).
	 */
	*(u32 *)sa_p->ips_j_ctx = sa_p->ips_said.spi;

	/* Original FreeSWAN's CryptoAPI supports only ESP protocol. To set AH
	 * protocol encryption key length is passes as -1.
	 */
	ret = ixt_j->ixt_j_set_key(sa_p->ips_authalg, sa_p->ips_encalg,
	        sa_p->ips_j_ctx, sa_p->ips_key_a, sa_p->ips_key_bits_a/8,
		sa_p->ips_said.proto == IPPROTO_AH ? NULL : sa_p->ips_key_e,
		sa_p->ips_said.proto == IPPROTO_AH ? -1 : sa_p->ips_key_bits_e/8,
...
/*
 * 	main joint auth and encrypt service entry point
 * 	called from ipsec_rcv() with encrypt=IPSEC_ALG_DECRYPT and
 * 	ipsec_tunnel_start_xmit with encrypt=IPSEC_ALG_ENCRYPT
 */
int ipsec_alg_joint_auth_and_encrypt(struct sk_buff *skb, struct ipsec_sa *sa_p,
	__u8 * idat, int ilen, int enc_offset, int authlen, const __u8 * iv,
	int encrypt, ipsec_alg_complete_cb_t complete_cb, void *cb_ctx)
{
	struct ipsec_alg_enc *ixt_e=sa_p->ips_alg_enc;
	struct ipsec_alg_joint *ixt_j=sa_p->ips_alg_joint;

	return ixt_j->ixt_j_auth_and_encrypt(skb, ixt_e, sa_p->ips_j_ctx,
	    idat, ilen, enc_offset, authlen, iv, encrypt, complete_cb, cb_ctx);
}
...
int ipsec_alg_sa_init(struct ipsec_sa *sa_p) {
	struct ipsec_alg_enc *ixt_e;
	struct ipsec_alg_auth *ixt_a;
	struct ipsec_alg_joint *ixt_j;

	if (sa_p->ips_said.proto != IPPROTO_ESP &&
	    sa_p->ips_said.proto != IPPROTO_AH)
	...
	if ((ixt_j=(struct ipsec_alg_joint *)
		ipsec_alg_get(IPSEC_ALG_TYPE_JOINT, 0))) {
	        /* If joint does not support AH protocol, then use original
		 * FreeSWAN's code instead.
		 */
	        if (sa_p->ips_said.proto == IPPROTO_AH &&
		    !ixt_j->ixt_j_is_support_ah_proto)
		{
		    ipsec_alg_put(ixt_j);
		    return 0;
		}
		KLIPS_PRINT(debug_pfkey,
		    "klips_debug: ipsec_alg_sa_init() :"
		    "found joint ipsec_alg (ixt_j=%p)\n", ixt_j);
		sa_p->ips_alg_joint=ixt_j;
	}
	return 0;
}
...
int ipsec_alg_sa_wipe(struct ipsec_sa *sa_p) {
	...
	if ((ixt=(struct ipsec_alg *)sa_p->ips_alg_joint)) {
		KLIPS_PRINT(debug_pfkey, "klips_debug: ipsec_alg_sa_wipe() :"
				"unlinking for joint alg\n");
		ipsec_alg_put(ixt);
	}
	return 0;
}
