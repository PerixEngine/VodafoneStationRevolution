...
static hash_table tunnels_by_addresses;
...
static void tunnel_handle_SCCRQ(int sock,
    					 l2tp_dgram *dgram,
					 EventSelector *es,
...
static int tunnel_rtx_delay_get(l2tp_tunnel *tunnel);
static int tunnel_max_retry_get(l2tp_tunnel *tunnel);
...
/* Callback to notify about new incoming connection request. */
int (*tunnel_new_request_cb)(l2tp_tunnel *, uint32_t, uint32_t, uint32_t);
/* Callback to notify about tunnel state: connected/disconnected. */
void (*tunnel_state_cb)(l2tp_tunnel *, int);

void tunnel_set_new_request_cb(int (*cb)(l2tp_tunnel *, uint32_t, uint32_t,
    uint32_t))
{
    tunnel_new_request_cb = cb;
}

void tunnel_set_tunnel_state_cb(void (*cb)(l2tp_tunnel *, int))
{
    tunnel_state_cb = cb;
}

l2tp_peer *(*tunnel_get_lns_peer)(l2tp_tunnel *, struct in_addr *peer_ip,
    uint16_t port);

void tunnel_set_get_lns_peer(l2tp_peer *(*cb)(l2tp_tunnel *, struct in_addr *,
    uint16_t))
{
    tunnel_get_lns_peer = cb;
}

static uint32_t (*tunnel_is_all_sessions_idle)(l2tp_tunnel *, uint32_t);

void tunnel_set_sessions_idle_cb(uint32_t (*cb)(l2tp_tunnel *, uint32_t))
{
    tunnel_is_all_sessions_idle = cb;
}
...
tunnel_xmit_queued_messages(l2tp_tunnel *tunnel)
{
	...
	if (l2tp_dgram_send_to_wire(tunnel->sock, dgram,
	    &tunnel->peer_addr) < 0)
	{
	    break;
	}
...
static unsigned int
tunnel_compute_peerlocalhash(void *data)
{
    return (unsigned int) (((l2tp_tunnel *)data)->peer_addr.sin_addr.s_addr) +
	(((l2tp_tunnel *)data)->local_addr.s_addr);
}

static int
tunnel_compare_peerlocal(void *d1, void *d2)
{
    return (((l2tp_tunnel *)d1)->peer_addr.sin_addr.s_addr) !=
	(((l2tp_tunnel *)d2)->peer_addr.sin_addr.s_addr) &&
    (((l2tp_tunnel *)d1)->local_addr.s_addr) !=
	(((l2tp_tunnel *)d2)->local_addr.s_addr);
}
...
l2tp_tunnel_init(EventSelector *es)
{
    ...
    hash_init(&tunnels_by_addresses,
	      offsetof(l2tp_tunnel, hash_by_peer),
	      tunnel_compute_peerlocalhash,
	      tunnel_compare_peerlocal);
...
static void
tunnel_hello_dgram_free(l2tp_tunnel *tunnel)
{
    if (!tunnel->hello_dgram)
	return;

    l2tp_dgram_free(tunnel->hello_dgram);
    tunnel->hello_dgram = NULL;
}
...
tunnel_free(l2tp_tunnel *tunnel)
{
    ...
    hash_remove(&tunnels_by_addresses, tunnel);
    ...
    tunnel_hello_dgram_free(tunnel);
    memset(tunnel, 0, sizeof(l2tp_tunnel));
    free(tunnel);
}
...
{
    l2tp_tunnel *tunnel;
    int sock = l2tp_get_fd_by_ip(peer->local_addr);
    ...
    tunnel->local_addr = peer->local_addr;
    tunnel->sock = sock;
    if (peer->start_rtx_delay)
	tunnel->timeout = peer->start_rtx_delay;

    hash_insert(&tunnels_by_addresses, tunnel);
...
***********************************************************************/
void
l2tp_tunnel_handle_received_control_datagram(int sock,
                                             l2tp_dgram *dgram,
					     EventSelector *es,
					     struct sockaddr_in *from)
{
	...
	tunnel_handle_SCCRQ(sock, dgram, es, from);
	return;
    }
    ...
    if (!(tunnel->timeout = tunnel_rtx_delay_get(tunnel)))
	tunnel->timeout = 1;
...
static l2tp_tunnel *tunnel_find_by_ip_port(struct sockaddr_in *from)
{
    void *cursor;
    l2tp_tunnel *tunnel;

    for (tunnel = hash_start(&tunnels_by_my_id, &cursor); tunnel;
        tunnel = hash_next(&tunnels_by_my_id, &cursor))
    {
        if (tunnel->peer_addr.sin_addr.s_addr == from->sin_addr.s_addr &&
            tunnel->peer_addr.sin_port == from->sin_port)
        {
            break;
        }
		    ...
		    struct sockaddr_in *from)
{
    ...
    /* If tunnel new request callback is not set, so the L2TP server is
     * disabled.
     */
    if (!tunnel_new_request_cb)
	return;

    if (tunnel_find_by_ip_port(from))
        return;
    ...
    tunnel->local_addr.s_addr = l2tp_get_ip_by_fd(sock);
    ...
    if (tunnel_set_params(tunnel, dgram) < 0) goto Error;

    if (tunnel_new_request_cb(tunnel, tunnel->my_id,
	tunnel->peer_addr.sin_addr.s_addr, l2tp_get_ip_by_fd(tunnel->sock)))
    {
	goto Error;
    }
    return;
...
void tunnel_reject_SCCRQ(uint32_t unaccept_id)
{
    l2tp_tunnel *tunnel = l2tp_tunnel_find_by_my_id((uint16_t)unaccept_id);

    if (tunnel)
	tunnel_free(tunnel);
}

void tunnel_accept_SCCRQ(uint32_t unaccept_id, void *peer_private)
{
    l2tp_dgram *dgram;
    uint16_t u16;
    uint32_t u32;
    ...
    if (!tunnel)
	return;
    tunnel->peer->private = peer_private;
...
static int
tunnel_rtx_delay_get(l2tp_tunnel *tunnel)
{
    if (!tunnel->peer)
	return 0;

    return tunnel->state == TUNNEL_ESTABLISHED ? tunnel->peer->hello_rtx_delay :
	tunnel->peer->start_rtx_delay;
}

static int
tunnel_max_retry_get(l2tp_tunnel *tunnel)
{
    if (!tunnel->peer)
	return 0;

    return tunnel->state == TUNNEL_ESTABLISHED ? tunnel->peer->hello_max_retry :
	tunnel->peer->start_max_retry;
}
...
{
    l2tp_tunnel *tunnel = (l2tp_tunnel *) data;
    int max_retry = tunnel_max_retry_get(tunnel) ? : 5;
    int rtx_delay = tunnel_rtx_delay_get(tunnel) ? : 1; 
    int step;
    ...
    tunnel->xmit_new_dgrams = tunnel->xmit_queue_head ? : tunnel->hello_dgram;

    /* Set Ns on wire to Ns of first frame in queue */
    if (tunnel->xmit_new_dgrams) {
	tunnel->Ns_on_wire = tunnel->xmit_new_dgrams->Ns;
    }

    if (max_retry && tunnel->retransmissions >= max_retry)
    {
	tunnel_free(tunnel);
	return;
    }
    ...
    step = tunnel->retransmissions < 4 ? tunnel->retransmissions : 4;
    tunnel->timeout = rtx_delay * (1 << (step - 1)); /* RFC behaviour */
    if (step > 1) 
    {
	/* Non-RFC flavor to spread out on a [-tout/2;+tout/2] interval */
	tunnel->timeout = (tunnel->timeout >> 1) +
	    rand() % (tunnel->timeout + 1);
    }
...
tunnel_send_ZLB(l2tp_tunnel *tunnel)
{
    ...
    dgram->Nr = tunnel->Nr;
    dgram->Ns = tunnel->Ns;
    l2tp_dgram_send_to_wire(tunnel->sock, dgram, &tunnel->peer_addr);
    l2tp_dgram_free(dgram);
}
		  ...
		  l2tp_dgram *dgram)
{
    ...
    /* Find peer */
    tunnel->peer = l2tp_peer_find(&tunnel->peer_addr, &tunnel->local_addr);
	...
	peer_ip.s_addr = tunnel->peer_addr.sin_addr.s_addr;
	if (tunnel_get_lns_peer)
	{
	    tunnel->peer = tunnel_get_lns_peer(tunnel, &peer_ip,
		ntohs(tunnel->peer_addr.sin_port));
	}
	if (!tunnel->peer)
	{
	    l2tp_set_errmsg("Peer %s is not authorized to create a tunnel",
		inet_ntoa(tunnel->peer_addr.sin_addr));
	    tunnel_send_StopCCN(tunnel, RESULT_NOAUTH, ERROR_OK, "%s",
		l2tp_get_errmsg());
	    return -1;
	}
    }
...
{
    l2tp_tunnel *tunnel = (l2tp_tunnel *) data;
    uint32_t min_idle = tunnel->peer->hello_delay ? : 60;
    ...
    /* Now the daemon does not handle data stream, so this event occurs if
     * there was no control message in last 60 seconds. So, need to check
     * data stream too. HELLO packet should be sent only when no control and
     * no data.
     */
    if (tunnel_is_all_sessions_idle &&
	!tunnel_is_all_sessions_idle(tunnel, min_idle))
    {
	tunnel_hello_dgram_free(tunnel);
	return;
    }
    ...
    tunnel->hello_dgram = l2tp_dgram_new_control(MESSAGE_HELLO,
	tunnel->assigned_id, 0);
    if (tunnel->hello_dgram)
    {
	tunnel->xmit_new_dgrams = tunnel->hello_dgram;
	tunnel->hello_dgram->Ns = tunnel->Ns;
        tunnel->Ns++;
	tunnel_xmit_queued_messages(tunnel);
    }
}
...
l2tp_tunnel_stop_invalid(void)
{
    ...
    t.tv_sec = 0;
    t.tv_usec = 0;
    /* Do not send StopCCN on tunnels with invalid socket. */
    for (tunnel = hash_start(&tunnels_by_my_id, &cursor);
	 tunnel;
	 tunnel = hash_next(&tunnels_by_my_id, &cursor))
    {
	if (tunnel->sock >= 0 && !l2tp_socket_is_valid(tunnel->sock))
	{
	    tunnel_set_state(tunnel, TUNNEL_RECEIVED_STOP_CCN);
	    tunnel_free(tunnel);
	}
...
void l2tp_tunnel_free_tunnel(l2tp_tunnel *tunnel)
{
    tunnel_free(tunnel);
}
/**********************************************************************
