...
typedef struct l2tp_peer_t {
    ...
    struct in_addr local_addr;	/* Local address to use for peer */
...
typedef struct l2tp_tunnel_t {
    ...
    int sock;			/* Tunnel socket */
    void (*close)(struct l2tp_tunnel_t *tunnel); /* close callback */
    l2tp_dgram *hello_dgram;    /* Hello message for retransmission */
} l2tp_tunnel;
...
typedef struct l2tp_call_ops_t {
    ...
    /* Called to add specific AVP */
    void (*add_avp)(l2tp_dgram *dgram, l2tp_session *ses);
} l2tp_call_ops;
...
void l2tp_tunnel_handle_received_control_datagram(int sock,
    						  l2tp_dgram *dgram,
						  EventSelector *es,
...
void l2tp_tunnel_stop_invalid(void);
...
l2tp_dgram *l2tp_dgram_take_from_wire(int sock, struct sockaddr_in *from);
int l2tp_dgram_send_to_wire(int sock, l2tp_dgram const *dgram,
...
int l2tp_socket_init(EventSelector *es, struct in_addr *srcs, int nsrcs);
int l2tp_socket_is_valid(int sock);
...
l2tp_peer *l2tp_peer_insert_and_fill(int is_lac, char *handler_name,
    struct in_addr *addr, uint16_t port, struct in_addr *local_addr,
    char *secret, int secret_len, void *private,
    void (*close_peer)(l2tp_peer *));
