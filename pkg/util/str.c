...
char **str_strip(char **s, char *char_set, int remove)
{
    char *p, *q, *news;
    int len;
    ...
    len = str_count_chrs(*s, char_set);
    news = malloc_e((remove ? strlen(*s) - len : len) + 1);

    for (p = *s, q = news; *p; p++)
    {
	if (!remove && strchr(char_set, *p))
	    *q++ = *p;
	else if (remove && !strchr(char_set, *p))
	    *q++ = *p;
    }
    *q = 0;
    free(*s);
...
{
    return str_strip(s, "0123456789", 0);
}
...
void str_buf_fastcat(str_buf_t *str_buf, char *val, char *separator)
{
    int val_len = str_len(val);
    int sep_len = str_len(separator);

    if (str_buf->buf_size - str_buf->data_len <= val_len + sep_len)
    {
	str_buf->buf_size = (str_buf->buf_size + val_len + sep_len) * 2;
	str_buf->buf = realloc_e(str_buf->buf, str_buf->buf_size);
    }

    strcpy(str_buf->buf + str_buf->data_len, val);
    str_buf->data_len += val_len;

    if (separator)
    {
	strcpy(str_buf->buf + str_buf->data_len, separator);
	str_buf->data_len += sep_len;
    }
}

int str_ends_with(char *s, char *ext)
{
    int s_len = strlen(s);
    int ext_len = strlen(ext);

    return ext_len <= s_len && !strcmp(&s[s_len - ext_len], ext);
}
