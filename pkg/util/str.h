...
typedef struct {
    ...
    int data_len; /* string length excluding trailing '\0' */
} str_buf_t;
...
/* Return 1 if all the characters of 's' are of ASCII character set */
...
/* if 'exclude' is 0 remove characters not found in 'char_set' from str.
 * if 'exclude' is 1 remove characters found in 'char_set' from str. */
char **str_strip(char **s, char *char_set, int exclude);
...
/* Reallocates two times larger buffer if the current one has not enough space
 * for cat */
