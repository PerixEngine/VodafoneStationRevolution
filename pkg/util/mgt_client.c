...
#define MAX_CLI_LINE 4096
int mgt_command_fmt(char **result, char *fmt, ...)
{
    static char cmd[MAX_CLI_LINE];
    va_list ap;

    va_start(ap, fmt);
    vsnprintf(cmd, MAX_CLI_LINE, fmt, ap);
    va_end(ap);

    return mgt_command(cmd, result);
}

void mgt_reboot_command(int linux_fallback)
{
    char *res = NULL;

    if (mgt_command("system reboot", &res) && linux_fallback)
