    ...
    const char *str_error, const char *format, va_list ap)
{
    ...
    sigset_t aset, oset;    
#else
    static DEFINE_SPINLOCK(rgerror_buf_lock);
...
static rg_error_itofmt_t *itofmt_lookup(rg_error_itofmt_t map[], int i)
{
    int idx;

    for (idx = 0; map[idx].i != -1; idx++)
    {
	if (map[idx].i == i)
	    return &map[idx];
    }
...
int rg_error_itofmt_set_enabled(rg_error_itofmt_t map[], int i, int enabled)
{
    rg_error_itofmt_t *p = itofmt_lookup(map, i);
...
int rg_error_itofmt_full(int entity_id, rg_error_itofmt_t map[], int i, ...)
{
    va_list ap;
    int ret = 0;
    rg_error_itofmt_t *p = itofmt_lookup(map, i);
    ...
    {
	va_start(ap, i);
	ret = _rg_vperror(entity_id, p->level, NULL, p->fmt, ap);
	va_end(ap);
    }
