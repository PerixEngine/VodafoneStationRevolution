...
#define AUTOGEN_MSG "OpenRG: This file was automatically generated by " \
    "pkg/build/create_config"
#define SEE_LOG_MSG "See %s for configuration log."

char *hw, *os = "", *dist, *features, *hw_compatible = NULL;
char *mk_file_name = NULL, *h_file_name = NULL, *c_file_name = NULL;
char *js_file_name = NULL;
static char *rgconf_key_file_name = NULL;
FILE *mk_file = NULL, *c_file = NULL, *h_file = NULL, *js_file = NULL;
#ifdef  CONFIG_RG_DO_DEVICES
char *dev_if_conf_file_name = NULL;
#endif
char *major_features_file = NULL, *major_features_cfile = NULL;
int is_evaluation, no_lic, is_default_license;
...
static void handle_opt(int argc, char *argv[])
{
    while (1)
    {
	switch (getopt(argc, argv, "d:f:h:o:c:i:m:e:M:F:s:r:"))
	{
	case 'd':
	    dist = optarg;
	    break;
...
static void print_ref(FILE *file, int avoid_print, option_t *o)
{
    if (avoid_print)
	return;
    
    if (o->set_prio == SET_PRIO_CMD_LINE)
	fprintf(file, "/* Set by command line */\n");
    else if (o->line)
	fprintf(file, "/* %s:%d */\n", o->file, o->line);
    else
    {
...
static void print_h_option(option_t *o)
{
	...
	print_ref(h_file, avoid_print_ref++, o);
...
static void print_js_option(option_t *o)
{
    char *v = o->value, *t = o->token;
    char *quote = o->type & OPT_C_STR ? "'" : "";
    int avoid_print_ref = 0, is_y_m;
    char *v_str = strdup("");
    ...
    {
	str_printf(&v_str, "%s",
	    (v && (!strcmp(v ,"y") || !strcmp(v ,"m"))) ? "true" : "false");
    }
    else
	str_cpy(&v_str, v ? "true" : "false");

    print_ref(js_file, avoid_print_ref++, o);
    fprintf(js_file, "var %s = %s;\n", t, v_str);
    str_free(&v_str);
}
...
static void configure_print(void)
{
    ...
    if (!h_file_name || !mk_file_name || !c_file_name || !js_file_name)
	conf_err("MUST define mk_file, h_file, c_file and js_file_name");
    ...
    if (!(js_file = fopen(js_file_name, "w")))
	conf_err("Can't open js_file: %s", js_file_name);

    fprintf(mk_file, "#" AUTOGEN_MSG "\n#" SEE_LOG_MSG "\n\n",
	getenv("CONFIG_LOG"));

    fprintf(c_file, "/* " AUTOGEN_MSG " \n * " SEE_LOG_MSG "\n */\n\n"
	"#include <stdlib.h>\n", getenv("CONFIG_LOG"));
    if (token_get("CONFIG_RG_CONFIG_STRINGS"))
	fprintf(c_file, "#include <build/config_strings.h>\n");
    fprintf(c_file, "\n");

    fprintf(h_file, "/* " AUTOGEN_MSG " \n * " SEE_LOG_MSG "\n */\n"
	"#ifndef _RG_CONFIG_H_\n"
	"#define _RG_CONFIG_H_\n\n"
	"#define AUTOCONF_INCLUDED\n", getenv("CONFIG_LOG"));
    
    fprintf(js_file, "/* OpenRG: This file was automatically generated by "
...
static code2str_t modules[] = {
	...
	{FEATURE_80211, "CONFIG_HW_BCM9636X_WLAN"},
	...
	{FEATURE_80211, "CONFIG_HW_80211N_RALINK_RT3883"},
	...
	{FEATURE_80211, "CONFIG_HW_80211N_BCM432X"},
	...
	{FEATURE_USB_HOST, "CONFIG_HW_USB_HOST_XHCI"},
...
static void license_init(void)
{
    ...
    /* Fetch the rg_conf crypto key from license */
    if (rgconf_key_file_name)
    {
	FILE *rgconf_key_file;

	output = sys_get(&rc, LIC_RG_APP " rgconf_key %s", lic_file);
	if (rc || str_isempty(output))
	    conf_err("failed to get rg_conf crypto key from the license\n");

	if (!(rgconf_key_file = fopen(rgconf_key_file_name, "w")))
	    conf_err("Can't create file: %s", rgconf_key_file_name);

	/* Delete \n symbol */
	output[strlen(output) - 1] = '\0';

	fprintf(rgconf_key_file, "/* " AUTOGEN_MSG "\n * " SEE_LOG_MSG "\n */\n"
	    "#ifndef _RG_CONF_KEY_H_\n"
	    "#define _RG_CONF_KEY_H_\n\n"
	    "#define RGCONF_CRYPTO_KEY \"%s\"\n\n"
	    "#endif\n",
	    getenv("CONFIG_LOG"), output);
	str_free(&output);
	fclose(rgconf_key_file);
    }
